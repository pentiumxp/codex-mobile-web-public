...
function buildRestartMacShellCommand(options = {}) {
  ...
  const envArgs = buildRestartEnvArgs(safeRestartEnvAssignments(options.env || process.env, {
    ...
    CODEX_MOBILE_DEFAULT_SHELL: targetDefaultShell,
  ...
  const commonLines = [
    ...
    "  fi",
    "}",
    "detect_launchd_service_domain() {",
    "  [[ -n \"$service_label\" ]] || return 0",
    "  local launchd_info",
    "  launchd_info=\"$(run_restart_sudo \"$launchctl_path\" print \"system/$service_label\" 2>/dev/null || true)\"",
    "  if [[ \"$launchd_info\" == *\"type = LaunchDaemon\"* ]]; then",
    "    service_domain=\"system/$service_label\"",
    "    printf '%s\\n' \"$launchd_info\"",
    "    return 0",
    "  fi",
    "  launchd_info=\"$($launchctl_path print \"$launchd_domain/$service_label\" 2>/dev/null || true)\"",
    ...
    "  fi",
    "}",
    "sync_system_launchdaemon_restart_env() {",
    ...
    "}",
    "reload_system_launchdaemon_service() {",
    "  [[ \"$service_domain\" == system/* ]] || return 1",
    "  [[ -n \"${service_plist_path:-}\" ]] || return 1",
    "  run_restart_sudo \"$launchctl_path\" bootout \"$service_domain\" >/dev/null 2>&1 || true",
    "  run_restart_sudo \"$launchctl_path\" bootstrap system \"$service_plist_path\" >/dev/null",
    "}",
    "restart_has_target_env() {",
    "  [[ -n \"$target_codex_home\" || -n \"$target_mux_endpoint_file\" || -n \"$target_default_shell\" ]]",
    "}",
    "restart_launchd_service() {",
    "  [[ -n \"$service_domain\" ]] || return 1",
    "  if [[ \"$service_domain\" == system/* ]]; then",
    "    if restart_has_target_env && [[ -n \"${service_plist_path:-}\" ]]; then",
    "      reload_system_launchdaemon_service",
    "      return $?",
    "    fi",
    "    run_restart_sudo \"$launchctl_path\" kickstart -k \"$service_domain\" >/dev/null 2>&1",
    "  else",
    "    \"$launchctl_path\" kickstart -k \"$service_domain\" >/dev/null 2>&1",
    "  fi",
    "}",
    "repair_system_launchdaemon_stdio() {",
    "  [[ -n \"$service_label\" ]] || return 0",
    "  local launchd_info",
    "  launchd_info=\"$(detect_launchd_service_domain)\"",
    "  [[ \"$launchd_info\" == *\"type = LaunchDaemon\"* ]] || return 0",
    "  local service_user",
    "  service_user=\"$(printf '%s\\n' \"$launchd_info\" | /usr/bin/awk -F' = ' '/username = / { print $2; exit }')\"",
    "  [[ -n \"$service_user\" ]] || service_user=\"$(/usr/bin/id -un)\"",
    "  while IFS= read -r log_path; do",
    "    [[ -n \"$log_path\" ]] || continue",
    "    local log_dir",
    "    log_dir=\"$(/usr/bin/dirname \"$log_path\")\"",
    "    run_restart_sudo /bin/mkdir -p \"$log_dir\"",
    "    run_restart_sudo /bin/chmod 711 \"$log_dir\"",
    "    run_restart_sudo /usr/bin/touch \"$log_path\"",
    "    run_restart_sudo /usr/sbin/chown \"$service_user:staff\" \"$log_path\"",
    "    run_restart_sudo /bin/chmod 600 \"$log_path\"",
    "  done < <(printf '%s\\n' \"$launchd_info\" | /usr/bin/awk -F' = ' '/std(out|err) path = / { print $2 }')",
    "}",
    "repair_system_launchdaemon_stdio >/dev/null 2>&1 || true",
    ...
    "selected_mux_endpoint_pids() {",
    "  [[ -n \"$target_mux_endpoint_file\" && -r \"$target_mux_endpoint_file\" ]] || return 0",
    "  \"$node_bin\" - \"$target_mux_endpoint_file\" <<'NODE'",
    "const fs = require('node:fs');",
    "const file = process.argv[2];",
    "let endpoint = {};",
    "try { endpoint = JSON.parse(fs.readFileSync(file, 'utf8')); } catch (_) {}",
    ...
    "  [[ -n \"$target_mux_endpoint_file\" ]] || return 0",
    "  local stopped=0",
    "  while IFS= read -r pid; do",
    "    [[ \"$pid\" =~ ^[0-9]+$ ]] || continue",
    "    local command",
    "    command=\"$(/bin/ps -p \"$pid\" -o command= 2>/dev/null || true)\"",
    "    if [[ \"$command\" == *\"codex-app-server-mux\"* || \"$command\" == *\"codex app-server\"* ]]; then",
    "      /bin/kill \"$pid\" >/dev/null 2>&1 || true",
    "      stopped=1",
    "    fi",
    "  done < <(selected_mux_endpoint_pids)",
    "  if [[ \"$stopped\" == \"1\" ]]; then",
    "    /bin/rm -f \"$target_mux_endpoint_file\" >/dev/null 2>&1 || true",
    "  fi",
    "}",
    "stop_selected_mux_endpoint >/dev/null 2>&1 || true",
  ];
  if (serviceLabel) {
    return commonLines.concat([
      ...
      "sync_system_launchdaemon_restart_env \"$service_plist_path\" >/dev/null",
      "if restart_launchd_service; then exit 0; fi",
      "if restart_has_target_env; then exit 1; fi",
  ...
  function restart(options = {}) {
    ...
    if (platform === "darwin") {
      ...
      const command = buildRestartMacShellCommand({
        ...
        defaultShellMode: options.defaultShellMode || deps.defaultShellMode,
      });
    ...
    const restartViaSystemHelper = options.systemTask === true
      || deps.systemTask === true
      || truthyEnv(env.CODEX_MOBILE_WINDOWS_SYSTEM_TASK)
      || truthyEnv(env.CODEX_MOBILE_RUN_AS_SYSTEM_TASK);
    if (!restartViaSystemHelper) {
