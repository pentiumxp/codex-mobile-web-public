...
test("thread running hints survive notLoaded list refreshes", () => {
  ...
  assert.match(functionBody("loadThread"), /applyThreadDetailRefreshPostMergeEffectsGroup\(postMergePlan, "merge"\);[\s\S]*const mergeMs = roundedDurationMs\(mergeStartedAt\);/);
  assert.match(functionBody("loadThread"), /const composerRenderMs = applyThreadDetailRefreshTimedPostMergeEffectsGroup\(postMergePlan, "composer-render"\);/);
  assert.match(functionBody("loadThread"), /const threadListRenderMs = applyThreadDetailRefreshTimedPostMergeEffectsGroup\(postMergePlan, "thread-list-render"\);/);
  ...
  assert.match(functionBody("refreshCurrentThread"), /const mergeMs = applyThreadDetailRefreshTimedPostMergeEffectsGroup\(postMergePlan, "merge", \{[\s\S]*startedAt: mergeStartedAt,[\s\S]*\}\);/);
  assert.match(functionBody("refreshCurrentThread"), /const composerRenderMs = applyThreadDetailRefreshTimedPostMergeEffectsGroup\(postMergePlan, "composer-render"\);/);
  assert.match(functionBody("refreshCurrentThread"), /const threadListRenderMs = applyThreadDetailRefreshTimedPostMergeEffectsGroup\(postMergePlan, "thread-list-render"\);/);
  ...
  assert.match(functionBody("applyThreadDetailRefreshResponseEffect"), /state\.currentThread = mergeThreadPreservingVisibleItems\(state\.currentThread, thread\);/);
  assert.match(functionBody("backfillFullThreadDetail"), /const mergeMs = applyThreadDetailRefreshTimedPostMergeEffectsGroup\(postMergePlan, "merge", \{[\s\S]*startedAt: mergeStartedAt,[\s\S]*\}\);/);
  assert.match(functionBody("backfillFullThreadDetail"), /const composerRenderMs = applyThreadDetailRefreshTimedPostMergeEffectsGroup\(postMergePlan, "composer-render"\);/);
  assert.match(functionBody("backfillFullThreadDetail"), /const threadListRenderMs = applyThreadDetailRefreshTimedPostMergeEffectsGroup\(postMergePlan, "thread-list-render"\);/);
  ...
  assert.match(functionBody("backfillFullThreadDetail"), /const fullBackfillPerformanceInput = threadDetailRenderPlanApi\.planThreadDetailFullBackfillPerformanceInput\(\{[\s\S]*source,[\s\S]*threadId: id,[\s\S]*timings: \{[\s\S]*elapsedMs: roundedDurationMs\(apiStartedAt\),[\s\S]*postRenderMs,[\s\S]*\},[\s\S]*\}\);/);
  ...
  assert.match(functionBody("backfillFullThreadDetail"), /const fullBackfillTelemetryPlan = threadDetailRenderPlanApi\.planThreadDetailFullBackfillTelemetryEffects\(\{[\s\S]*performanceEvent: fullReadyPerformance,[\s\S]*threadId: id,[\s\S]*\}\);[\s\S]*applyThreadDetailRefreshTelemetryEffectsPlan\(fullBackfillTelemetryPlan, \{ thread: result\.thread \}\);/);
