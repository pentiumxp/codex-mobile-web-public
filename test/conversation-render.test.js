...
test("current-thread refresh patches the current tile pane for metadata-only tile updates", () => {
  ...
  assert.match(body, /let patchAttemptResultStage = threadDetailRenderPlanApi\.planThreadDetailRefreshPatchAttemptResultStage\(\{/);
  assert.match(body, /patchAttempt,[\s\S]*renderPlan,[\s\S]*readMode: result\.thread && result\.thread\.mobileReadMode/);
  assert.match(body, /if \(patchAttemptResultStage\.needsPatchRejectedVisibleShapes\) \{/);
  assert.match(body, /previousVisibleShape: visibleConversationShape\(previousThread\)/);
  assert.match(body, /nextVisibleShape: visibleConversationShape\(state\.currentThread\)/);
