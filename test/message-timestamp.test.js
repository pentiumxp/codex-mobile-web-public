...
test("conversation cards render compact timestamps in the item header", () => {
  ...
  assert.match(appJs, /(?:const|var) timestampHtml = renderItemTimestampHtml\(item,\s*turn,\s*contextThread\)/);
...
test("card timestamps fall back from item time to turn time", () => {
  ...
  assert.match(appJs, /(?:const|var) contextThread = renderContextThread\(thread\);/);
...
test("running turn timestamps do not fall back to stale thread updated time", () => {
  ...
  assert.match(appJs, /(?:const|var) startedAt = turnStartedAtMs\(turn\);/);
