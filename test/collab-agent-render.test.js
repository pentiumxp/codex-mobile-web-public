...
test("live operation cards dock on wide screens and become a mobile bubble", () => {
  ...
  assert.match(functionBody("updateLiveOperationDockHtml"), /Date\.now\(\) \+ LIVE_OPERATION_BUBBLE_MIN_VISIBLE_MS/);
  ...
  assert.match(functionBody("updateLiveOperationDockHtml"), /state\.liveOperationDockCompactVisibleUntilMs = 0/);
  assert.match(functionBody("scheduleLiveOperationDockCompactMinimumRefresh"), /setTimeout\(\(\) => \{[\s\S]*renderCurrentThread\(\);[\s\S]*\}, delay \+ 16\)/);
