...
test("live operation cards dock on wide screens and become a mobile bubble", () => {
  ...
  assert.match(functionBody("rememberCompactLiveOperationBubbleHtml"), /state\.liveOperationDockCompactHtml = nextHtml/);
  assert.match(functionBody("rememberCompactLiveOperationBubbleHtml"), /state\.liveOperationDockCompactThreadId = threadId/);
  assert.match(functionBody("rememberCompactLiveOperationBubbleHtml"), /state\.liveOperationDockRecallHtml = nextHtml/);
  assert.match(functionBody("rememberCompactLiveOperationBubbleHtml"), /state\.liveOperationDockRecallThreadId = threadId/);
  assert.match(functionBody("renderLiveOperationRecallDockHtml"), /if \(!isMobileViewport\(\)\) return ""/);
  assert.match(functionBody("renderLiveOperationRecallDockHtml"), /savedThreadId !== String\(state\.currentThreadId \|\| ""\)/);
  ...
  assert.match(stylesCss, /@media \(max-width: 760px\)[\s\S]*\.live-operation-dock\[data-recall-visible="true"\]:not\(\[data-mode="expanded"\]\)\s*{[\s\S]*bottom:\s*calc\(var\(--composer-height, 92px\) \+ 2px\);/);
  ...
  assert.match(stylesCss, /@media \(max-width: 760px\)[\s\S]*\.mobile-operation-recall\s*{[\s\S]*display:\s*grid;[\s\S]*width:\s*32px;[\s\S]*height:\s*32px;/);
