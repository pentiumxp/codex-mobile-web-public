...
test("current-turn reply jump shares one floating slot with the bottom jump", () => {
  ...
  assert.match(appJs, /function scheduleUsageBackfillRefresh\(delay = 350, options = \{\}\)/);
  ...
  assert.match(appJs, /scheduleUsageBackfillRefresh\(120, \{ force: true \}\)/);
  assert.match(appJs, /refreshCurrentThread\(\{\s*source: "usage-backfill",\s*force: forceRefresh,\s*userInitiated: forceRefresh,\s*\}\)\.catch\(showError\);/);
  ...
  assert.match(functionBody("applyNotification"), /if \(!suppressAutomaticRefresh\) \{[\s\S]*scheduleUsageBackfillRefresh\(120, \{ force: true \}\);[\s\S]*scheduleLivePollIfNeeded\(1400\);[\s\S]*\}/);
...
test("manual conversation scroll pauses live auto-stick until the user returns to bottom", () => {
  ...
  assert.match(functionBody("refreshCurrentThread"), /const refreshSuppressionContext = \{\s*threadId,\s*userInitiated: options\.userInitiated === true \|\| options\.force === true,\s*\};/);
  assert.match(functionBody("refreshCurrentThread"), /if \(shouldSuppressAutomaticCurrentThreadRefresh\(source, refreshSuppressionContext\)\) return;/);
  ...
  assert.match(functionBody("scheduleUsageBackfillRefresh"), /const forceRefresh = options\.force === true \|\| options\.userInitiated === true;/);
  assert.match(functionBody("scheduleUsageBackfillRefresh"), /if \(shouldSuppressAutomaticCurrentThreadRefresh\("usage-backfill", suppressionContext\)\) return;/);
