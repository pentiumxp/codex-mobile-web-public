...
test("sidebar exposes a confirmed manual restart action beside the version pill", () => {
  ...
  assert.match(appUpdateRuntimeJs, /async function handleSharedRestartClick\(\)/);
  assert.match(appJs, /function handleSharedRestartClick\(\.\.\.args\) \{[\s\S]*requireAppUpdateRuntime\(\)\.handleSharedRestartClick\(\.\.\.args\)/);
  const restartBody = appUpdateRuntimeJs.slice(
    appUpdateRuntimeJs.indexOf("async function handleSharedRestartClick()"),
    appUpdateRuntimeJs.indexOf("function serverBuildIdFromConfig"),
  );
  ...
  assert.match(appUpdateRuntimeJs, /\/api\/restart\/shared-chain/);
...
test("manual restart distinguishes macOS Mobile Web restart scope", () => {
  ...
  assert.match(appUpdateRuntimeJs, /state\.serverPlatform === "darwin"/);
  assert.match(appUpdateRuntimeJs, /重启这台 Mac 上的 Mobile Web 服务/);
