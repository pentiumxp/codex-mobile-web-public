...
test("phase B readback decision returns ready for warm or bounded paths", () => {
  const decision = classifyPhaseBReadback({
    ...
    detail: {
      ...
      responseBudgetProgressiveActiveFirstPaintItemBudgetApplied: true,
  ...
  assert.equal(decision.evidence.detailResponseBudgetProgressiveActiveFirstPaintThreadByteCeiling, 98304);
  assert.equal(decision.evidence.detailResponseBudgetProgressiveActiveFirstPaintItemBudgetApplied, true);
...
test("phase B readback decision routes dominant route-merge latency to merge owner", () => {
  const decision = classifyPhaseBReadback({
    ...
    threadList: {
      ...
      fallbackRolloutStatusStatReuseCount: 12,
  ...
  assert.equal(decision.evidence.threadListFallbackRolloutStatusStatReadCount, 0);
  assert.equal(decision.evidence.threadListFallbackRolloutStatusStatReuseCount, 12);
