...
test("mobile viewport and early guards disable page zoom", () => {
  ...
  assert.match(viewportMetricsJs, /usesHostViewport/);
  ...
  assert.match(appJs, /const editableActive = isKeyboardEditableElement\(document\.activeElement\);/);
  assert.match(appJs, /if \(isHermesEmbedMode\(\)\) \{\s*\n\s*if \(editableActive \|\| pluginHostKeyboardVisible\(\)\) return;/);
  assert.match(appJs, /\} else if \(!editableActive\) \{\s*\n\s*return;\s*\n\s*\}/);
  assert.match(appJs, /if \(typeof window\.scrollTo === "function"\) window\.scrollTo\(0, 0\);/);
  assert.match(appJs, /if \(viewport\.keyboardShrunk \|\| viewport\.usesHostViewport\) \{[\s\S]*--app-height/);
  ...
  assert.match(appJs, /function pluginHostKeyboardVisible\(\)/);
