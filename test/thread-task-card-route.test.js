...
test("server materializes structured task-card drafts from thread detail", () => {
  ...
  assert.match(functionBody(serverJs, "prepareThreadDetailResponseResult"), /const detailResult = backfillMissingRolloutCompletionTurnsForDetailResult\(result, details\);/);
