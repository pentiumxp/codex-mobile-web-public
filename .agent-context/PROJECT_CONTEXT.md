...
- Thread detail reads skip expensive app-server detail RPCs and return local summary fallback once rollout size reaches `CODEX_MOBILE_THREAD_DETAIL_ROLLOUT_MAX_BYTES` (default equals `CODEX_MOBILE_ROLLOUT_WARNING_BYTES`). Detail RPC timeouts no longer reset the whole app-server connection.
