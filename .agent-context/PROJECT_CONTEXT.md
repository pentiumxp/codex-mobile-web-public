...
- `POST /api/threads` is the rollout "压缩续接" action. It starts a same-workspace app-server thread through `thread/start`, includes discovered `AGENTS.md` instructions as start-thread developer instructions, names the continuation from the source thread title plus a local `MM-DD` date suffix, sends a detailed bootstrap message, optionally archives the source thread after bootstrap succeeds, and returns the new thread id so the browser can switch directly.
- The rollout continuation bootstrap must carry enough detail for a fresh thread: source thread id/title/cwd/rollout path/rollout size/status, inherited runtime settings, recent visible source-turn summaries, `.agent-context/PROJECT_CONTEXT.md` excerpts, `.agent-context/HANDOFF.md` critical GitHub/release sections and latest tail, and explicit private/public/README release reminders. Do not rely only on a generic "read handoff" instruction.
