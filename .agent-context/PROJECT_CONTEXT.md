...
- `POST /api/threads` is the rollout "压缩续接" action. With a source thread id, it first starts a source-thread handoff turn that must write `.agent-context/thread-handoffs/<id>.md` in the current workspace, waits for that file, then starts the same-workspace continuation thread through `thread/start`, includes discovered `AGENTS.md` instructions as start-thread developer instructions, names the continuation from the source thread title plus a local `MM-DD` date suffix, sends a scoped bootstrap message, optionally archives the source thread after bootstrap succeeds, and returns the new thread id so the browser can switch directly.
...
- The browser should switch to the source thread before starting rollout continuation and remain on that source thread after the new continuation thread is created. The user should see the source-thread handoff turn and its result. A `thread/archived` event for the source thread during continuation must not clear the current detail view back to the home screen.
