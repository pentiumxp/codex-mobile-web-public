...
- Thread detail reads skip expensive app-server detail RPCs once rollout size reaches `CODEX_MOBILE_THREAD_DETAIL_ROLLOUT_MAX_BYTES` (default equals `CODEX_MOBILE_ROLLOUT_WARNING_BYTES`) and then prefer bounded `thread/turns/list` before falling back to local summary. Normal-size detail reads prefer full `thread/read` with `CODEX_MOBILE_FULL_THREAD_TURNS` (default `80`, capped at `200`) and fall back to `thread/turns/list` if full read fails. Detail RPC timeouts no longer reset the whole app-server connection.
...
- Continuation jobs first start a source-thread handoff turn that must write `.agent-context/thread-handoffs/<id>.md` in the current workspace, wait for that file, optionally wait longer via `CODEX_MOBILE_CONTINUATION_LATE_HANDOFF_TIMEOUT_MS`, reuse recent handoff files within `CODEX_MOBILE_CONTINUATION_REUSE_HANDOFF_MS`, wait briefly for source turn completion, then start the same-workspace continuation thread through `thread/start`, send a scoped bootstrap message, optionally archive the source thread, and return the new thread id so the browser can switch directly. The handoff directory gets a `.gitignore` to keep generated runtime handoff Markdown out of commits.
...
- The sidebar menu header includes a compact font-size selector (`small`, `default`, `large`, `xlarge`, `xxlarge`) stored in `localStorage` as `codexMobileFontSize`. It should stay in the menu/sidebar area, not the main conversation topbar, and it adjusts conversation text, markdown/code/table content, approval details, and composer input through CSS variables on `:root[data-font-size]`; `xxlarge` is the large-tablet readability option above `特大`.
...
- Thread detail reads prefer full app-server `thread/read includeTurns:true` only for normal-size rollout files; large rollout files skip that expensive path and try bounded `thread/turns/list` before local summary fallback, so the current detail view can still show recent turns when possible.
