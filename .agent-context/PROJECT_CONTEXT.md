...
- Thread detail reads prefer full app-server `thread/read includeTurns:true` only for normal-size rollout files; large rollout files skip that expensive path and use local summary / `thread/turns/list` fallback.
