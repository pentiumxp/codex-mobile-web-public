...
9. Render saved image uploads as bounded thumbnails from the upload/file preview route even when model context stays reference-only, including quoted `Uploaded attachments:` summaries in Codex/plan replies.
...
2. For ChatGPT Pro, inspect `C:\ProgramData\HermesMobile\data\chatgpt-pro-bridge-state.json` for the Codex thread id, then inspect that thread through Mobile Web.
