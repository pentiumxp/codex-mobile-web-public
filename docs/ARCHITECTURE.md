...
Runtime/detail active evidence such as `activeTurnId`, `mobileLocalActiveStatus`, or `mobileRuntimeDerived` active status is stronger than a non-live app-server list row during duplicate list-summary merge and display-cache list writeback. Ordinary `thread/list` rows recorded by `rememberList` are marked as list-origin summaries and cannot downgrade a cached runtime-active summary to idle/completed unless they carry matching active-turn evidence. Detail-authoritative summaries and terminal stale-active evidence such as `mobileClearedStaleActiveSummary`, `mobileClearedTerminalActiveTurn`, or `mobileClearedSupersededActiveTurn` are still allowed to clear active markers, so a genuinely completed thread does not stay permanently running. This lets Home AI and deploy-lane rows keep the running indicator when app-server reports a newer idle/completed row while detail or fallback evidence still shows a live turn.
