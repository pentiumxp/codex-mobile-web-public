...
`initial=warm-fallback` when there is no search or workspace filter, so the
already-prewarmed process baseline can paint before the app-server refresh
arrives. That policy lives in `public/thread-list-load-policy.js`; search,
workspace-filtered, and explicitly full list requests remain on the full
authoritative path. The server-side display-summary cache also skips
read-time rollout-stat decoration on cache reads when the list merge already
has request-scoped rollout/session metadata.
