...
reasoning, command-output, and file-output deltas. Cache signatures include the
rollout size/mtime, summary updated time/status, the retained turn window, and
the projection policy version; stale signatures miss, while live in-memory
projection entries are accepted only when their notification timestamp is not
older than the current summary. If projection misses, the route still prefers
