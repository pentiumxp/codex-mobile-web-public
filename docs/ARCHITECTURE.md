...
The profile store, current listener runtime home, and app-server/mux binding are
separate observable states. `codexProfiles.activeCodexHome` is the selected
profile home from the store; `codexProfiles.runtimeCodexHome` and
`codexProfiles.runtimeState` show whether the current listener has already
aligned. App-server RPCs such as `thread/start` and `turn/start` check the
runtime binding before sending: if the active profile store changed while the
old listener process is still alive, Mobile Web closes the stale transport and
connects to the selected profile's mux endpoint or starts a Mobile-owned mux for
that profile. Rate-limit state is keyed by the same active home and clears old
live/snapshot quota when the profile key changes, so a previous account's
exhausted quota cannot block a newly selected account.
...
Runtime/detail active evidence such as `activeTurnId`, `mobileLocalActiveStatus`, or `mobileRuntimeDerived` active status is stronger than a non-live app-server list row during duplicate list-summary merge and display-cache list writeback. Ordinary `thread/list` rows recorded by `rememberList` are marked as list-origin summaries and cannot downgrade a cached runtime-active summary to idle/completed unless they carry matching active-turn evidence. Detail-authoritative summaries and terminal stale-active evidence such as `mobileClearedTerminalActiveTurn` or `mobileClearedSupersededActiveTurn` are still allowed to clear active markers, so a genuinely completed thread does not stay permanently running. A `summary-resting-active-window` downgrade from a partial projection is not detail authority by itself; it must not clear runtime-active evidence until a fresh full/detail or terminal source proves the turn settled. This lets Home AI and deploy-lane rows keep the running indicator when app-server reports a newer idle/completed row while detail or fallback evidence still shows a live turn.
...
`thread_list_slow_path` from the same `thread_list_rendered` event fields when
elapsed, API, or render time exceeds the bounded list threshold. The diagnostic
payload is built by `public/thread-diagnostic-events.js` and contains only safe
phase labels and counters such as fallback-cache decision, app-server request
reason, server/client timing splits, result count, and source/app-server
counters. Fast successful list loads clear the repeated-failure signature; slow
...
item budget to reduce successful-but-heavy first paints. Historical completed
turns keep the latest assistant/plan receipt, active turns keep a bounded recent
assistant/plan tail, and when no turn is active the latest completed turn keeps
a bounded recent operation/reasoning/assistant replay tail. `mobileDetailResponseBudget.omittedAssistantItems`
...
Large-session list first paint is separate from thread-detail projection
authority. The browser's default first thread-list request can use
`initial=warm-fallback` when there is no search or workspace filter, so either
the already-prewarmed process baseline or a cold local fallback baseline can
paint before the app-server refresh arrives. That policy lives in
`public/thread-list-load-policy.js`; search, workspace-filtered, and explicitly
...
Scheduled PR handling uses a separate source-side automation boundary:
`scripts/codex-mobile-pr-automation.js` runs one bounded planning pass and
delegates all policy to `services/runtime/pr-automation-service.js`. It may
discover private/public PRs, record bounded `.agent-context` state, and emit
task-card/deploy/public-ready next-step requests with deterministic idempotency
keys, but it must not directly merge, deploy, public-push, or close PRs. Public
push and PR close remain gated on deploy/readback and public-ready evidence.
