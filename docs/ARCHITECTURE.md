...
Runtime/detail active evidence such as `activeTurnId`, `mobileLocalActiveStatus`, or `mobileRuntimeDerived` active status is stronger than a non-live app-server list row during duplicate list-summary merge. This lets Home AI and deploy-lane rows keep the running indicator when app-server reports a newer idle/completed row while detail or fallback evidence still shows a live turn.
