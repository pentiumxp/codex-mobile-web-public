...
| Bottom Command/status row disappears during a running turn | Check whether the latest live turn is reasoning-only or has an active operational item. Clients after `codex-mobile-shell-v393` keep a synthetic `liveTurnStatus` row visible for reasoning-only active turns so the dock height stays stable until the turn completes. |
