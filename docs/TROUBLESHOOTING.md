...
| Bottom Command/status row disappears during a running turn | Check whether the latest live turn is reasoning-only or has an active operational item. Clients after `codex-mobile-shell-v394` keep a synthetic `liveTurnStatus` placeholder row visible for reasoning-only active turns so the dock height stays stable until the turn completes; the placeholder does not display `思考`, because the top-right timer already owns that status. |
| Command row has no detail on macOS | Inspect whether the operation item has `command` directly or a serialized `arguments` object with `command`/`cmd`/`shellCommand`. Clients after `codex-mobile-shell-v394` read both shapes before rendering the dock detail. |
