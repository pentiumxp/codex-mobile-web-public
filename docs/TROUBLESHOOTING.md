...
| Final receipt appears, disappears, then returns one line shorter | Compare the live active receipt text and completed service projection receipt text for the same turn. Current clients after `codex-mobile-shell-v391` treat a completed authoritative `agentMessage`/`plan` that is a stable same-prefix shorter version as the same render identity, preserving the existing visible node/text while merging Usage. |
