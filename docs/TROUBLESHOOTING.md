...
| Light endpoints jitter while listener CPU/RSS is high | Read authenticated `/api/status?detail=1` and inspect `runtimePressure.eventLoop`, `runtimePressure.process`, and `runtimePressure.routes` before sampling private data. Current builds keep `/api/public-config` and `/api/status` from waiting on quota/app-server hydration; quota refresh and rollout snapshot scans run as bounded background work. If `runtimePressure.routes` shows slow light endpoints, inspect route composition and synchronous string/regexp/body scans before blaming app-server transport. |
