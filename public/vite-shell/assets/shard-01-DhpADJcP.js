import { i as __toESM, r as __commonJSMin } from "./vite-shell-entry-BFpMLiiz.js";
...
function threadTurnCount(thread) {
	return Array.isArray(thread && thread.turns) ? boundedCount$1(thread.turns.length) : 0;
}
function threadOmittedTurnCount(thread) {
	return boundedCount$1(thread && thread.mobileOmittedTurnCount);
}
...
function planThreadDetailSlowPathDiagnostic(event = {}, input = {}) {
	...
	return {
		...
		turns: boundedCount$1(fields.turns || detailShape.turns),
		visibleItems: boundedCount$1(detailShape.visibleItems),
		omittedTurns: boundedCount$1(fields.omittedTurns || detailShape.omittedTurns)
	};
}
function planThreadListSlowPathDiagnostic(event = {}, input = {}) {
	...
	return {
		...
		durationBucket: compactLabel(source.durationBucket || source.duration_bucket, 40),
		performancePhase,
		count: boundedCount$1(fields.count || source.count),
		...
		fallbackRolloutFileStatCount: boundedCount$1(serverTimings.fallbackRolloutFileStatCount),
		fallbackRolloutHeadReadCount: boundedCount$1(serverTimings.fallbackRolloutHeadReadCount),
		fallbackRolloutSummaryReadCount: boundedCount$1(serverTimings.fallbackRolloutSummaryReadCount),
		appServerRequestLimit: boundedCount$1(serverTimings.appServerRequestLimit),
		appServerResponsePayloadKb: Number.isFinite(responseBytes) && responseBytes > 0 ? boundedCount$1(Math.ceil(responseBytes / 1024)) : 0
	};
}
function threadDetailProjectionContractFields(thread) {
	...
	return {
		...
		responseBudgetActiveTurnCount: boundedCount$1(responseBudget.activeTurnCount),
		responseBudgetRetainedItemCount: boundedCount$1(responseBudget.retainedItemCount),
...
function planThreadDetailResponseContractDiagnostic(event = {}, input = {}) {
	...
	const responseBudgetActiveTurnCount = boundedCount$1(contract.responseBudgetActiveTurnCount || source.responseBudgetActiveTurnCount);
	const responseBudgetRetainedItemCount = boundedCount$1(contract.responseBudgetRetainedItemCount || source.responseBudgetRetainedItemCount);
	...
	const turns = boundedCount$1(fields.turns || contract.turns || detailShape.turns);
	const items = boundedCount$1(detailShape.items);
	const visibleItems = boundedCount$1(detailShape.visibleItems);
	const activeTurns = boundedCount$1(detailShape.activeTurns);
	const completedTurns = boundedCount$1(detailShape.completedTurns);
	const omittedTurns = boundedCount$1(fields.omittedTurns || contract.omittedTurns || detailShape.omittedTurns);
...
function threadDetailShape(thread) {
	...
	const shape = {
		turns: boundedCount$1(turns.length),
		omittedTurns: boundedCount$1(thread && thread.mobileOmittedTurnCount),
		items: 0,
		visibleItems: 0,
		userItems: 0,
		receiptItems: 0,
	...
	for (const key of Object.keys(shape)) shape[key] = boundedCount$1(shape[key]);
	return shape;
}
...
function planSummaryOnlyCurrentThreadRecoveryEffects(plan = {}) {
	const recoveryPlan = objectOrEmpty$1(plan);
	const effects = [];
...
var import_thread_detail_dom_patch = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
var moduleDefinitions = [
	...
	},
	{
		...
		"importSource": "public/thread-detail-dom-patch.js",
