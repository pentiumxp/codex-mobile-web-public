...
function planSingleThreadFullRenderShell(input = {}) {
	...
	const body = hasPrimaryContent ? `${text(input.taskCardReceiptsHtml)}${text(input.turnsHtml)}${text(input.approvalsHtml)}${text(input.taskCardsHtml)}${text(input.pluginRefreshNotice)}` : `${text(input.pluginRefreshNotice)}<div class="empty-state entry-animate">${escape(emptyMessage)}</div>`;
	return {
		mode: "detail",
