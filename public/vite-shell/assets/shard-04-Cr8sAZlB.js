...
function closeAppNativeDialog$1(confirmed = false) {
...
function requestAppNativeDialog(options = {}) {
	if (state.appNativeDialogResolve) closeAppNativeDialog$1(false);
	const mode = [
		"alert",
		"confirm",
		"prompt"
...
function handleAppNativeDialogKeydown$1(event) {
	if (!state.appNativeDialogOpen) return;
	if (event.key === "Escape") {
		event.preventDefault();
		closeAppNativeDialog$1(false);
		return;
	}
	if ((event.metaKey || event.ctrlKey) && event.key === "Enter") {
		event.preventDefault();
		closeAppNativeDialog$1(true);
	}
}
...
function closeCodexProfileSwitchDialog$1(confirmed = false) {
...
function requestCodexProfileSwitchConfirmation(profileId, label) {
	if (state.profileSwitchConfirmResolve) closeCodexProfileSwitchDialog$1(false);
...
function startCodexProfileSwitchProgressPolling(requestId) {
	...
	const poll = async () => {
		...
		try {
			const result = await api$1(`/api/codex-profiles/switch-progress?requestId=${encodeURIComponent(id)}`, { timeoutMs: 5e3 });
...
async function performCodexProfileSwitch(profileId) {
	...
	try {
		const result = await api$1("/api/codex-profiles/active", {
			method: "POST",
			body: JSON.stringify({
				profileId,
				requestId
		...
		try {
			const progressResult = await api$1(`/api/codex-profiles/switch-progress?requestId=${encodeURIComponent(requestId)}`, { timeoutMs: 5e3 });
...
var api$1 = { createModalRuntime };
Object.assign(root$2, {
	renderAppNativeDialog,
	closeAppNativeDialog: closeAppNativeDialog$1,
	requestAppNativeDialog,
	requestAppAlert,
	requestAppConfirmation,
	requestAppTextInput,
	handleAppNativeDialogKeydown: handleAppNativeDialogKeydown$1,
	renderCodexProfileSwitchDialog,
	closeCodexProfileSwitchDialog: closeCodexProfileSwitchDialog$1,
...
root$2.CodexModalRuntime = api$1;
//#endregion
...
function isLiveReasoning(item, turn, thread = null) {
	return item && item.type === "reasoning" && isLatestTurn$1(turn, thread) && isLiveTurn$1(turn, thread) && !isCompletedStatus(item.status);
}
...
function turnHasActiveLiveItems$1(turn) {
	return (Array.isArray(turn && turn.items) ? turn.items : []).some((item) => item && (item.type === "reasoning" && !isCompletedStatus(item.status) || isActiveOperationalItem$1(item)));
}
...
function startRelativeTimeTimer$1() {
...
Object.assign(root$1, {
	...
	updateOperationDurationBadges,
	startRelativeTimeTimer: startRelativeTimeTimer$1,
	threadSignature,
...
//#endregion
//#region frontend/native/app-shell-runtime.mjs
...
var appShellApi = Object.freeze({ createAppShellRuntime });
var appShellRoot = typeof globalThis !== "undefined" ? globalThis : window;
appShellRoot.CodexAppShellRuntime = appShellApi;
//#endregion
