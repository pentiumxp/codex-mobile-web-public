//#region frontend/native/build-refresh-policy.mjs
...
function titleCaseModelSuffix(value) {
	return String(value || "").split(/[-_]+/).map((part) => part ? part.charAt(0).toUpperCase() + part.slice(1) : "").filter(Boolean).join(" ");
}
...
function applyModelOptionsRefresh(state, config = {}) {
	...
	if (!optionsChanged && !defaultChanged) return {
		changed: false,
...
//#endregion
//#region frontend/native/viewport-metrics.mjs
var DEFAULT_KEYBOARD_SHRINK_PX = 120;
var DEFAULT_MIN_HEIGHT = 320;
var DEFAULT_STABLE_PIXEL_EPSILON_PX = 1;
...
var viewport_metrics_default = {
	cssPixel,
	isKeyboardEditable,
	measureViewport,
	stablePixelChanged
};
var DEFAULT_SUBMIT_FOLLOW_MS = 1800;
var DEFAULT_VIEWPORT_FOLLOW_MS = 3200;
var DEFAULT_RECENT_BOTTOM_MS = 12e4;
var DEFAULT_BOTTOM_FOLLOW_DELAYS_MS = Object.freeze([
	0,
	80,
	240,
	600,
...
function isNearBottom(metrics = {}, thresholdPx = 96) {
...
function createSubmittedMessageFollow(threadId, options = {}) {
	...
	const ttlMs = Math.max(1e3, numberOrZero(options.ttlMs) || 1800);
	return {
		threadId: id,
...
function shouldStartViewportFollow(options = {}) {
	...
	const nowMs = numberOrZero(options.nowMs) || Date.now();
	const recentBottomMs = Math.max(0, numberOrZero(options.recentBottomMs) || 12e4);
...
function createViewportFollow(threadId, options = {}) {
	...
	if (!id) return null;
	const nowMs = numberOrZero(options.nowMs) || Date.now();
	const ttlMs = Math.max(500, numberOrZero(options.ttlMs) || 3200);
	return {
		threadId: id,
...
function planFullRenderScroll(options = {}) {
	...
	if (Boolean(options.submittedMessageFollow || options.viewportFollow)) return {
		stickToBottom: true,
		explicitNoStickToBottom: false,
...
//#endregion
//#region frontend/native/thread-performance-metrics.mjs
function objectOrNull(value) {
	return value && typeof value === "object" && !Array.isArray(value) ? value : null;
}
var MAX_TIMING_MS = 600 * 1e3;
var CLIENT_TIMING_KEYS = [
	"elapsedMs",
	"apiElapsedMs",
	"renderElapsedMs",
	"mergeMs",
...
var ALLOWED_DETAIL_RENDER_MODES = Object.freeze({
	"cached-current": true,
	"first-paint": true,
	"full-backfill": true,
	"full-render": true,
...
function threadTurnCount(thread) {
	return Array.isArray(thread && thread.turns) ? boundedCount$2(thread.turns.length) : 0;
}
function threadOmittedTurnCount(thread) {
	return boundedCount$2(thread && thread.mobileOmittedTurnCount);
}
...
function planThreadDetailSlowPathDiagnostic(event = {}, input = {}) {
	...
	return {
		...
		turns: boundedCount$2(fields.turns || detailShape.turns),
		visibleItems: boundedCount$2(detailShape.visibleItems),
		omittedTurns: boundedCount$2(fields.omittedTurns || detailShape.omittedTurns)
	};
}
function planThreadListSlowPathDiagnostic(event = {}, input = {}) {
	...
	return {
		...
		durationBucket: compactLabel(source.durationBucket || source.duration_bucket, 40),
		performancePhase,
		count: boundedCount$2(fields.count || source.count),
		...
		fallbackRolloutFileStatCount: boundedCount$2(serverTimings.fallbackRolloutFileStatCount),
		fallbackRolloutHeadReadCount: boundedCount$2(serverTimings.fallbackRolloutHeadReadCount),
		fallbackRolloutSummaryReadCount: boundedCount$2(serverTimings.fallbackRolloutSummaryReadCount),
		appServerRequestLimit: boundedCount$2(serverTimings.appServerRequestLimit),
		appServerResponsePayloadKb: Number.isFinite(responseBytes) && responseBytes > 0 ? boundedCount$2(Math.ceil(responseBytes / 1024)) : 0
	};
}
function threadDetailProjectionContractFields(thread) {
	...
	return {
		...
		responseBudgetActiveTurnCount: boundedCount$2(responseBudget.activeTurnCount),
		responseBudgetRetainedItemCount: boundedCount$2(responseBudget.retainedItemCount),
...
function planThreadDetailResponseContractDiagnostic(event = {}, input = {}) {
	...
	const responseBudgetActiveTurnCount = boundedCount$2(contract.responseBudgetActiveTurnCount || source.responseBudgetActiveTurnCount);
	const responseBudgetRetainedItemCount = boundedCount$2(contract.responseBudgetRetainedItemCount || source.responseBudgetRetainedItemCount);
	...
	const turns = boundedCount$2(fields.turns || contract.turns || detailShape.turns);
	const items = boundedCount$2(detailShape.items);
	const visibleItems = boundedCount$2(detailShape.visibleItems);
	const activeTurns = boundedCount$2(detailShape.activeTurns);
	const completedTurns = boundedCount$2(detailShape.completedTurns);
	const omittedTurns = boundedCount$2(fields.omittedTurns || contract.omittedTurns || detailShape.omittedTurns);
	...
	const activeOverlayProjectionMode = /projection-active-overlay/.test(readMode) || /active-overlay-window/.test(projectionPartialKind);
	const partialProjectionMode = projectionPartial || activeOverlayProjectionMode || /projection-v?\d*-partial|projection-partial/.test(readMode);
	...
	const activePartialProjectionOk = !source.expectedActiveFullRead && partialProjectionMode && hasActiveProjectionEvidence && hasVisibleProjectionEvidence && (responseBudgetApplied || responseBudgetProgressiveActiveApplied || activeOverlayProjectionMode || /warm-projection-partial|projection-partial/.test(performancePhase));
	const partialProjectionBackfillOk = !source.expectedActiveFullRead && fullBackfillPlanned && partialProjectionMode && hasVisibleProjectionEvidence;
...
function threadDetailShape(thread) {
	...
	const shape = {
		turns: boundedCount$2(turns.length),
		omittedTurns: boundedCount$2(thread && thread.mobileOmittedTurnCount),
		items: 0,
		visibleItems: 0,
		userItems: 0,
		receiptItems: 0,
	...
	for (const key of Object.keys(shape)) shape[key] = boundedCount$2(shape[key]);
	return shape;
}
...
//#endregion
//#region frontend/native/thread-detail-state.mjs
var DETAIL_ONLY_SUMMARY_FIELDS = Object.freeze([
	"turns",
	"runtimeSettings",
	"threadTaskCards",
...
function objectOrEmpty$2(value) {
...
function emptyDetailHistoryEvidenceForThread(thread) {
	...
	const omittedTurns = boundedCount$1(thread && thread.mobileOmittedTurnCount);
	...
	const pendingTaskCardCount = boundedCount$1(thread && thread.pendingTaskCardCount);
	const hasActiveTurnEvidence = Boolean(thread && (thread.activeTurnId || thread.mobileRolloutActiveTurn));
	const hasConversationEvidence = rolloutSizeBytes > 0 || omittedTurns > 0 || visibleItemKeyCount > 0 || hasActiveTurnEvidence;
	return {
		hasEvidence: hasConversationEvidence,
		hasConversationEvidence,
		hasTaskCardOnlyEvidence: !hasConversationEvidence && (taskCardCount > 0 || pendingTaskCardCount > 0),
		rolloutSizeBytes,
		omittedTurns,
		visibleItemKeyCount,
...
function buildThreadDetailRenderEvidence(input = {}) {
	...
	const turnCount = boundedCount$1(input.turnCount);
	const visibleItemCount = boundedCount$1(input.visibleItemCount);
	...
	return {
		...
		turnCount,
		visibleItemCount,
		itemCount: boundedCount$1(input.itemCount)
	};
}
function recentThreadDetailRenderEvidence(input = {}) {
	...
	return Object.assign({}, evidence, {
		ageMs,
		turnCount: boundedCount$1(evidence.turnCount),
		visibleItemCount: boundedCount$1(evidence.visibleItemCount),
		itemCount: boundedCount$1(evidence.itemCount)
	});
}
...
function threadUpdatedAtMs(thread) {
	...
	return timestampMs(thread.mobileListUpdatedAtMs) || timestampMs(thread.mobile_list_updated_at_ms) || timestampMs(thread.listActivityAtMs) || timestampMs(thread.list_activity_at_ms) || timestampMs(thread.updatedAtMs) || timestampMs(thread.updatedAt) || timestampMs(thread.updated_at_ms) || timestampMs(thread.updated_at) || timestampMs(thread.lastActivityAtMs) || timestampMs(thread.lastActivityAt) || timestampMs(thread.last_activity_at_ms) || timestampMs(thread.last_activity_at) || 0;
}
...
function hasNonemptyThreadDetailRenderEvidence(evidence) {
	return Boolean(evidence && (boundedCount$1(evidence.turnCount) || boundedCount$1(evidence.visibleItemCount)));
}
...
function planSummaryOnlyCurrentThreadRecoveryEffects(plan = {}) {
	const recoveryPlan = objectOrEmpty$2(plan);
	const effects = [];
...
function createThreadDetailStatePolicy(options = {}) {
	...
	function incomingTurnHasUserMessage(incomingTurn = null) {
		return Array.isArray(incomingTurn && incomingTurn.items) && incomingTurn.items.some((item) => item && item.type === "userMessage");
	}
	function isPendingSubmittedUserMessage(item) {
		return Boolean(item && item.type === "userMessage" && !item.mobileSendError && (item.mobilePendingSubmission || String(item.clientSubmissionId || "").trim()));
	}
	function shouldPreserveLocalOnlyItem(item, preserveLocalVisible = false, suppressedVisualReceiptKeys = null, incomingTurn = null) {
		...
		if (isPendingSubmittedUserMessage(item) && completedIncomingTurnHasAuthoritativeReceipt(incomingTurn) && !incomingTurnHasUserMessage(incomingTurn)) return true;
...
//#endregion
//#region frontend/native/thread-detail-render-plan.mjs
...
function normalizedStringList$1(value) {
...
function turnOrderMismatch(expectedValue, renderedValue) {
	const expected = normalizedStringList$1(expectedValue);
	const rendered = normalizedStringList$1(renderedValue);
...
function objectOrEmpty$1(value) {
	return value && typeof value === "object" && !Array.isArray(value) ? value : {};
}
...
function itemVisibleText(item) {
	...
	const value = objectOrEmpty$1(item);
	return [
...
function planThreadDetailHistoryAutoBackfill(input = {}) {
	const thread = objectOrEmpty$1(input.thread);
...
function planThreadDetailHistoryAutoBackfillEffects(input = {}) {
	const plan = objectOrEmpty$1(input.plan);
		...
		effects: [
			...
			},
			{
				...
				payload: {
					...
					counts: objectOrEmpty$1(plan.counts),
...
function planThreadDetailRefreshRequest(input = {}) {
	const options = objectOrEmpty$1(input.options);
...
function planThreadDetailRefreshConsistencyCheckEffects(input = {}) {
	const consistencyCheck = objectOrEmpty$1(input.consistencyCheck || input);
...
function planThreadDetailRefreshRenderInput(input = {}) {
	const nextVisibleShape = objectOrEmpty$1(input.nextVisibleShape);
	...
	return {
		...
		expectedTurnIds: normalizedStringList$1(input.expectedTurnIds),
		renderedDomTurnIds: normalizedStringList$1(input.renderedDomTurnIds)
	};
}
...
function planThreadDetailRefreshPatchSurfaceProbeEffects(input = {}) {
	const patchSurfacePlan = objectOrEmpty$1(input.patchSurfacePlan || input.plan);
...
function planThreadDetailRefreshPatchSurfaceResultStage(input = {}) {
	const tilePatchPlan = objectOrEmpty$1(input.tilePatchPlan);
...
function planThreadDetailRefreshPatchSurfaceExecutionStage(input = {}) {
	const renderPlan = objectOrEmpty$1(input.renderPlan);
...
function planThreadDetailRefreshPatchExecutionStage(input = {}) {
	const renderPlan = objectOrEmpty$1(input.renderPlan);
	const patchSurfacePlan = objectOrEmpty$1(input.patchSurfacePlan);
	const shouldRenderDetail = Object.prototype.hasOwnProperty.call(input, "shouldRenderDetail") ? Boolean(input.shouldRenderDetail) : Boolean(renderPlan.shouldRenderDetail);
...
function threadDetailRefreshPatchAttemptEffectContext(context = {}, aggregate = {}) {
	return Object.assign({}, objectOrEmpty$1(context), { tilePanePatchedDetail: Boolean(aggregate && aggregate.tilePanePatchedDetail) });
}
function reduceThreadDetailRefreshPatchAttempt(aggregate = {}, attempt = {}) {
	const result = Object.assign(emptyThreadDetailRefreshPatchAttempt(), objectOrEmpty$1(aggregate));
	const patchAttempt = objectOrEmpty$1(attempt);
...
function planThreadDetailRefreshPatchAttemptResult(input = {}) {
	...
	const patchRejectReason = reportLocalPatchRejected ? compactReason(input.patchRejectReason, "unknown") : "";
	const finalizeResult = {
		locallyPatchedDetail,
		tilePanePatchedDetail
	};
	if (patchRejectReason === "patch-shell-changed") finalizeResult.patchRejectReason = patchRejectReason;
	return {
		patchResult,
		locallyPatchedDetail,
		tilePanePatchedDetail,
...
function visibleItemCountFromShape(shape, fallback = 0) {
	const value = objectOrEmpty$1(shape);
...
function planThreadDetailRefreshPatchRejectedDiagnostic(input = {}) {
	const patchAttemptResult = objectOrEmpty$1(input.patchAttemptResult);
...
function planThreadDetailRefreshPatchRejectedDiagnosticEffects(input = {}) {
	const diagnosticPlan = objectOrEmpty$1(input.diagnosticPlan || input.plan);
	...
	return {
		effects: [{
			...
			diagnosticInput: objectOrEmpty$1(diagnosticPlan.diagnosticInput)
		}],
		reason: "local-patch-rejected-diagnostic"
	};
}
function hasOwnPropertyValue(object, key) {
	return Object.prototype.hasOwnProperty.call(objectOrEmpty$1(object), key);
}
function planThreadDetailRefreshPatchRejectedVisibleShapeEvidenceEffects(input = {}) {
	const stage = objectOrEmpty$1(input.patchAttemptResultStage || input.stage);
...
function planThreadDetailRefreshPatchAttemptResultStage(input = {}) {
	const patchAttempt = objectOrEmpty$1(input.patchAttempt);
...
function planThreadDetailRefreshPatchAttemptResultEvidenceCompletionStage(input = {}) {
	const visibleShapeEvidence = objectOrEmpty$1(input.visibleShapeEvidence);
...
function planThreadDetailRefreshPatchAttemptResultEvidenceResolutionStage(input = {}) {
	const patchAttemptResultStage = objectOrEmpty$1(input.patchAttemptResultStage);
	const visibleShapeEvidence = objectOrEmpty$1(input.visibleShapeEvidence);
...
function finalizeThreadDetailRenderPlan(plan = {}, result = {}) {
	...
	if (patchRejectReason === "patch-shell-changed") return {
...
function planThreadDetailRefreshOutcomeExecution(outcome = {}) {
	...
	if (renderAction === "shell-patch-render") return {
		renderAction,
		metadataUpdateMode: "",
		metadataEffects: [],
...
function planThreadDetailRefreshOutcomeExecutionStage(input = {}) {
	const renderOutcome = finalizeThreadDetailRenderPlan(objectOrEmpty$1(input.renderPlan), objectOrEmpty$1(input.patchAttemptResult).finalizeResult);
...
function planThreadDetailRefreshPerformanceInput(input = {}) {
	const renderPlan = objectOrEmpty$1(input.renderPlan);
	const renderOutcome = objectOrEmpty$1(input.renderOutcome);
	const patchAttemptResult = objectOrEmpty$1(input.patchAttemptResult);
	const patchSurfacePlan = objectOrEmpty$1(input.patchSurfacePlan);
	const patchExecutionPlan = objectOrEmpty$1(input.patchExecutionPlan);
	const timings = objectOrEmpty$1(input.timings);
	return {
...
function planThreadDetailFirstPaintReportingStage(input = {}) {
	...
	const performanceInput = planThreadDetailFirstPaintPerformanceInput({
		...
		cached,
		fullBackfillPlanned: input.fullBackfillPlanned === true,
		timings: objectOrEmpty$1(input.timings)
	});
	return {
		performanceInput,
		telemetryInput: {
...
function planThreadDetailRefreshTelemetryEffects(input = {}) {
	const performanceEvent = objectOrEmpty$1(input.performanceEvent);
	const eventName = compactReason(input.eventName, "thread_refresh_ms");
...
function planThreadDetailRefreshReportingEffectsStage(input = {}) {
	const telemetryConfig = objectOrEmpty$1(input.telemetryConfig);
	const completionConfig = objectOrEmpty$1(input.completionConfig);
	return {
...
function planThreadDetailRefreshExecutionEffects(input = {}) {
	...
	if (executionAction === "shell-patch-render") return {
		effects: [{
		...
		effects: [
			{
				type: "post-performance-event",
				eventName: "thread_detail_first_paint",
				payload: objectOrEmpty$1(input.performanceEvent)
			},
			{
				type: "post-client-event",
...
function planSingleThreadFullRenderShell(input = {}) {
	...
	const body = hasPrimaryContent ? `${text(input.turnsHtml)}${text(input.taskCardReceiptsHtml)}${text(input.approvalsHtml)}${text(input.taskCardsHtml)}${text(input.pluginRefreshNotice)}` : `${text(input.pluginRefreshNotice)}<div class="empty-state entry-animate">${escape(emptyMessage)}</div>`;
	return {
		mode: "detail",
...
function planSingleThreadEarlyShellExecution(input = {}) {
	...
	if (Boolean(loadingWithoutVisibleTurns && !loadError && threadId && renderedThreadId === threadId && (renderedDomTurnCount > 0 || renderedDomItemCount > 0))) return {
		shouldRender: false,
		mode: "detail",
...
function planSingleThreadShellConversationUpdate(input = {}) {
	const shellPlan = objectOrEmpty$1(input.shellPlan);
...
//#endregion
//#region frontend/native/thread-detail-dom-patch.mjs
var ELEMENT_NODE = 1;
var TEXT_NODE = 3;
var COMMENT_NODE = 8;
...
function removeUnexpectedVisibleItemNodes(article, expectedKeys) {
	...
	for (const key of normalizedStringList(expectedKeys)) remaining.set(key, Number(remaining.get(key) || 0) + 1);
...
function userMessageDomDuplicateSignature(node) {
	const text = nodeText(Array.from(node && node.childNodes || []).find((child) => hasClasses(child, ["item-body"])) || node).replace(/\s+/g, " ").trim();
...
function userMessageDomNodePriority(node) {
	...
	return /^local-user\b/.test(item) || /(?:^|[|:])local-user/.test(renderKey) || /(?:^|[|:])mux-user/.test(renderKey) || Boolean(submissionHash) ? 0 : 1;
}
function removeDuplicateUserMessageDomNodes(input = {}) {
	...
	for (const article of turnArticleNodes(root)) {
		...
		for (const node of userMessageItemNodes(article)) {
			...
			const removeNode = userMessageDomNodePriority(node) > currentPriority ? current : node;
...
function applyVisibleItemRefreshDomPatch(input = {}) {
	...
	const expectedKeys = patchPlan.operations.map((operation) => normalizeOperation(operation)).filter(Boolean).map((operation) => operation.key);
	...
	if (!visibleItemOrderMatches(article, expectedKeys)) return result(false, "post-apply-visible-item-order-mismatch", counts);
...
if (root) root.CodexThreadDetailDomPatch = threadDetailDomPatchApi;
//#endregion
...
var moduleApis = {
	"build-refresh-policy": build_refresh_policy_default,
	"runtime-settings": runtime_settings_default,
	"viewport-metrics": viewport_metrics_default,
	"conversation-scroll": api$3,
	"thread-performance-metrics": api$2,
	"thread-detail-state": api$1,
	"thread-detail-render-plan": api,
	"thread-detail-dom-patch": threadDetailDomPatchApi
};
