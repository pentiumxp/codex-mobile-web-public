    ...
    `;
			...
			async function refreshPageForNewBuild() {
				...
				try {
					...
					try {
						latestConfig = reconnectRefresh ? await waitForPageBuildConfig() : await fetchPageBuildConfig();
					} catch (err) {
					...
					if (!config) {
						postPageRefreshEvent("reload-without-config", { errorCode: safeErrorCode(latestConfigError, "config_unavailable") });
