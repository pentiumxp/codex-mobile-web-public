...
root$2.CodexNotificationUiRuntime = notificationUiRuntimeApi;
//#endregion
...
function itemTimestampMs(item, turn = null, thread = null) {
	...
	if (item.type === "agentMessage" || item.type === "plan") return numericTimestampMs(item.completedAtMs) || numericTimestampMs(item.completedAt) || numericTimestampMs(item.completed_at_ms) || numericTimestampMs(item.completed_at) || turnCompletedAtMs(turn, contextThread) || (isLiveTurn(turn, contextThread) ? 0 : turnStartedAtMs$1(turn)) || 0;
	if (isLiveTurn(turn, contextThread) && isOperationalItem(item)) return turnStartedAtMs$1(turn) || 0;
	return turnStartedAtMs$1(turn) || turnCompletedAtMs(turn, contextThread);
}
function turnStartedAtMs$1(turn) {
	if (!turn) return 0;
...
function shouldDeferLiveFinalReceipt$1(turn, itemType) {
	return false;
}
function createConversationRenderRuntime() {
	return {
		...
		ensureTurn: typeof ensureTurn$1 === "function" ? ensureTurn$1 : null,
		shouldDeferLiveFinalReceipt: typeof shouldDeferLiveFinalReceipt$1 === "function" ? shouldDeferLiveFinalReceipt$1 : null
	};
}
...
Object.assign(conversationRenderRoot, {
	...
	itemTimestampMs,
	turnStartedAtMs: turnStartedAtMs$1,
	renderLiveReasoning,
	labelForItem,
	copyTextForItem,
	...
	ensureTurn: ensureTurn$1,
	shouldDeferLiveFinalReceipt: shouldDeferLiveFinalReceipt$1,
	createConversationRenderRuntime
});
conversationRenderRoot.CodexConversationRenderRuntime = conversationRenderRuntimeApi;
//#endregion
//#region frontend/native/event-stream-runtime.mjs
...
function isTrailingPendingUserEcho(item) {
	return Boolean(item && item.type === "userMessage" && !item.mobileSendError && isOptimisticUserMessage(item));
}
...
function insertLiveItem(turn, item) {
	...
	while (firstTrailingPendingIndex > 0 && isTrailingPendingUserEcho(turn.items[firstTrailingPendingIndex - 1])) firstTrailingPendingIndex -= 1;
...
function upsertItem(turnId, item) {
	...
	if (item.type === "userMessage") {
		const matchingExistingIndex = turn.items.findIndex((existing) => existing && existing.id !== item.id && existing.type === "userMessage" && userMessagesAreSameTurnDuplicateEvent(existing, item));
	...
	} else structureChanged = insertLiveItem(turn, item) || structureChanged;
	normalizeThreadVisibleUserMessages(state.currentThread);
...
function ensureEventConnection() {
	if (!state.key) return;
	if (!state.events || state.events.readyState === EventSource.CLOSED) connectEvents$1();
}
...
function scheduleVisualRecovery$1(reason = "visual", delay = 0, options = {}) {
	if (document.visibilityState === "hidden") return;
...
function scrollConversationToBottom() {
	...
	if (Math.abs(el.scrollTop - target) < 2) {
		scheduleScrollToBottomButtonUpdate$1();
		return;
	}
	markProgrammaticConversationScroll$1();
	el.scrollTop = target;
...
function markProgrammaticConversationScroll$1() {
...
function numericTimestampMs$1(value) {
	if (!value) return 0;
	const number = Number(value);
...
function turnIdentityTimestampMs$1(turn) {
	return uuidV7TimestampMs(turn && (turn.id || turn.turnId || turn.turn_id));
}
function turnCompletedAtMs$1(turn, thread = null) {
	if (!turn) return 0;
	const explicitCompletedAt = numericTimestampMs$1(turn.completedAtMs) || numericTimestampMs$1(turn.completedAt) || numericTimestampMs$1(turn.completed_at_ms) || numericTimestampMs$1(turn.completed_at) || numericTimestampMs$1(turn.finishedAt) || numericTimestampMs$1(turn.finished_at);
	if (explicitCompletedAt) return explicitCompletedAt;
	if (!isTurnComplete(turn)) return 0;
	const startedAt = turnStartedAtMs(turn);
	const fallback = numericTimestampMs$1(turn.updatedAt) || numericTimestampMs$1(turn.updated_at) || (turnIdentityTimestampMs$1(turn) ? 0 : numericTimestampMs$1(thread && (thread.updatedAt || thread.updated_at)));
...
function scheduleScrollToBottomButtonUpdate$1() {
...
function createEventStreamRuntime() {
	return {
		connectEvents: typeof connectEvents$1 === "function" ? connectEvents$1 : null,
...
var eventStreamRuntimeApi = Object.freeze({ createEventStreamRuntime });
var eventStreamRoot = typeof globalThis !== "undefined" ? globalThis : window;
Object.assign(eventStreamRoot, {
	...
	forceVisualRecovery,
	scheduleVisualRecovery: scheduleVisualRecovery$1,
	...
	markProgrammaticConversationScroll: markProgrammaticConversationScroll$1,
	clearConversationNearBottomState,
	clearConversationUserScrollAwayState,
	rememberConversationUserScrollAwayState,
	...
	numericTimestampMs: numericTimestampMs$1,
	uuidV7TimestampMs,
	turnIdentityTimestampMs: turnIdentityTimestampMs$1,
	turnCompletedAtMs: turnCompletedAtMs$1,
	...
	updateScrollToBottomButton,
	scheduleScrollToBottomButtonUpdate: scheduleScrollToBottomButtonUpdate$1
});
eventStreamRoot.CodexEventStreamRuntime = eventStreamRuntimeApi;
//#endregion
//#region frontend/native/client-render-stability-guard.mjs
...
root$1.CodexClientRenderStabilityGuard = api$2;
//#endregion
//#region frontend/native/live-operation-dock-state.mjs
...
if (root) root.CodexLiveOperationDockState = api$1;
//#endregion
...
var moduleApis = {
	...
	"event-stream-runtime": eventStreamRuntimeApi,
	"client-render-stability-guard": api$2,
	"live-operation-dock-state": api$1
};
