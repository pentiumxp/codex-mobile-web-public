...
function renderCodexProfileSettings$1() {
	...
	el.innerHTML = profiles.map((profile) => {
		...
		return `<div class="codex-profile-row${active ? " active" : ""}"><div class="codex-profile-main"><strong>${escapeHtml$1(profile.label || id)}</strong><span>${escapeHtml$1(codexProfileAccountLabel(profile))}</span><small>${escapeHtml$1(profile.codexHome || "")}</small>` + status + `</div><div class="codex-profile-side"><span class="codex-profile-quota">${escapeHtml$1(quotaShortTextFromSnapshot(profile.quota))}</span><button type="button" data-codex-profile-id="${escapeHtml$1(id)}" ${disabled ? "disabled" : ""} title="${escapeHtml$1(title)}">${escapeHtml$1(action)}</button></div></div>`;
	}).join("");
}
...
function renderRemoteManagedWorkspaceSettings() {
	...
	const tokenText = config.enrollmentTokenConfigured ? config.enrollmentTokenRef || config.enrollmentTokenPreview || "configured" : "not configured";
	el.innerHTML = `<div class="remote-managed-workspace-row${config.enabled ? " enabled" : ""}"><div class="remote-managed-workspace-main"><strong>${escapeHtml$1(config.enabled ? "已开启" : "已关闭")} · ${escapeHtml$1(status)}</strong><span>${escapeHtml$1(config.workspaceId || "workspace id")} / ${escapeHtml$1(config.nodeName || "node name")}</span><small>${escapeHtml$1(config.effectiveConnectionMode || "http_polling")} · token ${escapeHtml$1(tokenText)}${escapeHtml$1(issueText)}</small></div><div class="remote-managed-workspace-side"><button type="button" data-rmw-action="poll-once" ${busy || !config.enabled ? "disabled" : ""}>轮询</button></div></div><div class="remote-managed-workspace-form"><label class="remote-managed-workspace-check"><input type="checkbox" data-rmw-enabled ${config.enabled ? "checked" : ""}> <span>启用</span></label><label class="remote-managed-workspace-field"><span>Workspace kind</span><select data-rmw-field="workspaceKind"><option value="remote_managed_workspace" ${config.workspaceKind === "remote_managed_workspace" ? "selected" : ""}>remote_managed_workspace</option></select></label>` + remoteManagedWorkspaceFieldHtml("Central URL", "centralUrl", config.centralUrl, { placeholder: "https://home-ai.example" }) + remoteManagedWorkspaceFieldHtml("Workspace id", "workspaceId", config.workspaceId) + remoteManagedWorkspaceFieldHtml("Node name", "nodeName", config.nodeName) + remoteManagedWorkspaceFieldHtml("Project root", "projectRoot", config.projectRoot) + remoteManagedWorkspaceFieldHtml("Allowed root", "allowedRoot", config.allowedRoot) + remoteManagedWorkspaceFieldHtml("Project type", "projectType", config.projectType || "vite_game") + `<label class="remote-managed-workspace-field"><span>Connection</span><select data-rmw-field="connectionMode"><option value="persistent" ${config.connectionMode === "persistent" ? "selected" : ""}>persistent</option><option value="http_polling" ${config.connectionMode === "http_polling" ? "selected" : ""}>http polling</option></select></label>` + remoteManagedWorkspaceFieldHtml("Token ref", "enrollmentTokenRef", config.enrollmentTokenRef) + remoteManagedWorkspaceFieldHtml("Enrollment token", "enrollmentToken", "", {
		type: "password",
		placeholder: config.enrollmentTokenConfigured ? "configured" : ""
	}) + remoteManagedWorkspaceFieldHtml("Roles", "roles", config.roles.join(", ")) + remoteManagedWorkspaceFieldHtml("Capabilities", "capabilities", config.capabilities.join(", ")) + `<div class="remote-managed-workspace-actions"><button type="button" data-rmw-action="save" ${busy ? "disabled" : ""}>${busy ? "保存中" : "保存"}</button><button type="button" data-rmw-action="test-connection" ${busy ? "disabled" : ""}>测试连接</button><button type="button" data-rmw-action="register" ${busy || !config.enabled ? "disabled" : ""}>注册</button></div><div class="remote-managed-workspace-meta"><span>heartbeat ${escapeHtml$1(config.lastHeartbeatAt || "--")}</span><span>poll ${escapeHtml$1(config.lastPollAt || "--")}</span><span>task ${escapeHtml$1(config.lastTaskCardId || "--")}</span><span>return ${escapeHtml$1(config.lastReturnStatus || "--")}</span></div></div>`;
}
function remoteManagedWorkspaceFormPayload() {
	const el = $("remoteManagedWorkspaceSettings");
	if (!el) return {};
	const payload = { enabled: Boolean(el.querySelector("[data-rmw-enabled]") && el.querySelector("[data-rmw-enabled]").checked) };
	el.querySelectorAll("[data-rmw-field]").forEach((field) => {
		const name = field.getAttribute("data-rmw-field");
		if (!name) return;
		if (name === "roles" || name === "capabilities") {
			payload[name] = normalizeRemoteManagedWorkspaceList(field.value);
			return;
		}
...
async function handleRemoteManagedWorkspaceSettingsClick(event) {
	...
	const endpoint = action === "save" ? "/api/settings/remote-managed-workspace" : action === "test-connection" ? "/api/settings/remote-managed-workspace/test-connection" : action === "register" ? "/api/settings/remote-managed-workspace/register" : action === "poll-once" ? "/api/settings/remote-managed-workspace/poll-once" : "";
	if (!endpoint) return;
	state.remoteManagedWorkspaceBusy = true;
	renderRemoteManagedWorkspaceSettings();
	try {
		rememberRemoteManagedWorkspaceConfig(remoteManagedWorkspaceConfigFromResult(await api(endpoint, {
			method: "POST",
			body: action === "save" ? JSON.stringify(remoteManagedWorkspaceFormPayload()) : "{}",
			timeoutMs: 2e4
		})));
