import { i as __toESM, r as __commonJSMin } from "./vite-shell-entry-BP0qdzPl.js";
//#endregion
//#region frontend/native/composer-bridge-runtime.mjs
var import_composer_runtime = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
async function answerServerRequest(requestId, payload, options = {}) {
	...
	try {
		const result = await api$6(`/api/approvals/${encodeURIComponent(key)}`, {
			method: "POST",
...
function answerApproval(requestId, decision, options = {}) {
	...
	return api$6(`/api/approvals/${encodeURIComponent(key)}`, {
		method: "POST",
...
async function mutateThreadTaskCard(cardId, action, body = {}, options = {}) {
	...
	try {
		const result = await api$6(`/api/thread-task-cards/${encodeURIComponent(id)}/${encodeURIComponent(action)}`, {
			method: "POST",
...
async function createThreadTaskCardDraft(draftKey, options = {}) {
	...
	try {
		...
		const result = await api$6("/api/thread-task-cards", {
			method: "POST",
			body: JSON.stringify({
...
async function syncExistingPushSubscription() {
	...
	if (subscription) await api$5("/api/push/subscribe", {
		method: "POST",
...
async function enablePushNotifications() {
	...
	if (!subscription) {
		const key = await api$5("/api/push/vapid-public-key");
		subscription = await registration.pushManager.subscribe({
			userVisibleOnly: true,
			applicationServerKey: base64UrlToUint8Array(key.publicKey)
		});
	}
	await api$5("/api/push/subscribe", {
		method: "POST",
		body: JSON.stringify({ subscription: pushSubscriptionToJson(subscription) })
	});
...
async function sendTestPushNotification() {
	const result = await api$5("/api/push/test", {
		method: "POST",
		body: "{}"
	});
	$("connectionState").classList.remove("error");
...
var api = { createThreadDetailMergePolicy };
//#endregion
...
var moduleDefinitions = [
	{
		...
		"importSource": "public/composer-runtime.js",
...
var moduleApis = {
	...
	"composer-bridge-runtime": api$6,
	"api-client-runtime": apiClientRuntimeApi,
	"thread-list-load-policy": thread_list_load_policy_default,
	"thread-list-stable-order": api$4,
	"thread-status-hints": api$3,
	"thread-detail-patch-plan": api$2,
	"thread-detail-actions": api$1,
	"thread-detail-merge-state": api
};
