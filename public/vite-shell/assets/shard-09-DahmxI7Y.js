...
var api$2 = { createThreadDetailMergePolicy };
//#endregion
...
var api$1 = { createThreadDetailV4MergePolicy };
//#endregion
...
function renderThreadTaskCardReturnReceipt(card, previousKeys = /* @__PURE__ */ new Set(), threadId = "") {
	...
	const detailBlocks = [detail ? `<pre class="approval-detail">${escapeHtml(detail)}</pre>` : "", body];
	return `<section class="history-note thread-task-card-return-receipt${entryAnimationClass(key, previousKeys)} ${escapeHtml(status)}" data-render-key="${escapeHtml(key)}" data-task-card-return-receipt="${escapeHtml(card.id)}">
    <div class="approval-title">${escapeHtml(taskCardDirectionLabel(card))}</div>
    <div class="approval-method">${escapeHtml(title)}</div>
    ${summary ? `<div class="approval-summary-line">${escapeHtml(summary)}</div>` : ""}
...
function renderThreadTaskCardReturnReceipts(thread, previousKeys = /* @__PURE__ */ new Set()) {
    ...
    ${receipts.map((card) => renderThreadTaskCardReturnReceipt(card, previousKeys, threadId)).join("")}
  </div>`;
}
...
root$1.CodexNotificationUiRuntime = notificationUiRuntimeApi;
//#endregion
...
var moduleApis = {
	"thread-detail-actions": api$3,
	"thread-detail-merge-state": api$2,
	"thread-detail-v4-merge-state": api$1,
