...
var MODEL_OPTIONS_REFRESH_MIN_INTERVAL_MS = 5e3;
function applyRuntimeModelOptionsConfig(config = {}, source = "public-config", options = {}) {
	const apply = runtimeSettings && typeof runtimeSettings.applyModelOptionsRefresh === "function" ? runtimeSettings.applyModelOptionsRefresh : null;
	if (!apply) return {
		changed: false,
...
async function fetchPublicConfigForModelOptions() {
	...
	if (typeof fetchJsonWithTimeout === "function") return fetchJsonWithTimeout(`/api/public-config?${suffix}`, {
		timeoutMs: 12e3,
		cache: "no-store"
	});
...
async function refreshModelOptionsFromPublicConfig(options = {}) {
	...
	state.modelOptionsRefreshInFlight = fetchPublicConfigForModelOptions().then((config) => applyRuntimeModelOptionsConfig(config, options.source || "model-options-refresh")).catch((err) => {
...
function handleComposerRuntimeControlWithModelRefresh(event, kind, button) {
	...
	refreshModelOptionsFromPublicConfig({
...
function wireUi() {
	...
	document.addEventListener("visibilitychange", () => {
		...
		if (document.visibilityState === "visible") {
			...
			scheduleModelOptionsRefresh(250, {
				source: "visibility",
				force: true
			});
		}
	...
	window.addEventListener("pageshow", (event) => {
		...
		ensureAndroidBackToSidebarSentinel();
		schedulePageRefreshCheck(200, { force: true });
		scheduleModelOptionsRefresh(260, {
			source: "pageshow",
			force: true
		});
...
async function start() {
	...
	applyRuntimeModelOptionsConfig(config, "startup", {
		render: false,
		saveDraft: false
	});
