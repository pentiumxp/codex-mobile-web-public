...
function renderQuotaDetailPanel(fiveHour, weekly, model) {
	...
	panel.hidden = false;
	panel.innerHTML = [
		`<div class="quota-detail-title"><span>额度</span><strong>${escapeHtml$1(model ? labelForModel(model) : "当前模型")}</strong></div>`,
...
function remoteManagedWorkspaceRowStatus(config, workspace) {
	...
	if (status === "连接中" || status === "正在请求配对" || status === "等待 Home AI 审批") return {
		label: status,
		className: "connecting"
	};
	if (status === "离线/重试中") return {
		label: status,
		className: "offline"
	};
	if (status === "已拒绝" || status === "认证失败") return {
		label: status,
		className: "auth"
	};
	if (status === "已批准/已配对") return {
		label: status,
		className: "remote"
	};
	...
	if (status === "配置无效") return {
		label: "Invalid path",
		className: "invalid"
	};
	return {
		label: status || "Remote managed",
		className: "remote"
	};
}
...
function remoteManagedWorkspaceAdvancedHtml(config) {
	...
	return "<details class=\"remote-managed-workspace-advanced\"><summary>Advanced / Diagnostics</summary><div class=\"remote-managed-workspace-advanced-grid\">" + remoteManagedWorkspaceReadonlyField("workspace kind", config.workspaceKind) + remoteManagedWorkspaceReadonlyField("workspace id", config.workspaceId) + remoteManagedWorkspaceReadonlyField("node name", config.nodeName) + remoteManagedWorkspaceReadonlyField("project root", config.projectRoot) + remoteManagedWorkspaceReadonlyField("allowed root", config.allowedRoot) + remoteManagedWorkspaceReadonlyField("project type", config.projectType) + remoteManagedWorkspaceReadonlyField("pairing", config.pairingStatus) + remoteManagedWorkspaceReadonlyField("pairing request", config.pairingRequestId) + remoteManagedWorkspaceReadonlyField("pairing check", config.lastPairingCheckAt) + remoteManagedWorkspaceReadonlyField("connection", `${config.connectionMode} -> ${config.effectiveConnectionMode}`) + remoteManagedWorkspaceReadonlyField("credential", credentialText) + remoteManagedWorkspaceReadonlyField("roles", config.roles.join(", ")) + remoteManagedWorkspaceReadonlyField("capabilities", config.capabilities.join(", ")) + remoteManagedWorkspaceReadonlyField("heartbeat", config.lastHeartbeatAt) + remoteManagedWorkspaceReadonlyField("poll", config.lastPollAt) + remoteManagedWorkspaceReadonlyField("task", config.lastTaskCardId) + remoteManagedWorkspaceReadonlyField("return", config.lastReturnStatus) + remoteManagedWorkspaceReadonlyField("queued returns", String(config.queuedTerminalReturnCount || 0)) + remoteManagedWorkspaceReadonlyField("issues", issueText) + "</div></details>";
}
...
async function handleRemoteManagedWorkspaceSettingsClick(event) {
	...
	try {
		...
		$("connectionState").textContent = action === "register" ? `Remote Managed Workspace ${latestStatus}` : "Remote Managed Workspace 已保存";
	} catch (err) {
		showError(err);
...
root$2.CodexSettingsRuntime = settingsRuntimeApi;
//#endregion
...
root$1.CodexModalRuntime = modalRuntimeApi;
//#endregion
...
function shouldPollCurrentThread() {
	...
	return Boolean(state.activeTurnId) || isRunningStatus$1(turn.status) || isIncompleteInterruptedTurn(turn);
}
...
function isLiveTurn(turn, thread = null) {
	...
	return isRunningStatus$1(turn && turn.status) || isIncompleteInterruptedTurn(turn) || turnHasActiveLiveItems(turn) || isLatestTurn(turn, thread) && currentThreadHasActiveRuntimeStatus(thread);
}
...
function isLiveReasoning(item, turn, thread = null) {
	return item && item.type === "reasoning" && isLatestTurn(turn, thread) && isLiveTurn(turn, thread) && !isCompletedStatus$1(item.status);
}
...
function isLiveTurnForThread(thread, turn) {
	if (!turn || isTurnComplete$1(turn) || isStaleOrSupersededLiveTurn(turn)) return false;
	if (turnHasNewerDisplayTurn(thread, turn)) return false;
	return isRunningStatus$1(turn && turn.status) || isIncompleteInterruptedTurn(turn) || turnHasActiveLiveItems(turn) || latestTurnForThread(thread) === turn && isRunningStatus$1(thread && thread.status);
}
...
function turnHasActiveLiveItems(turn) {
	return (Array.isArray(turn && turn.items) ? turn.items : []).some((item) => item && (item.type === "reasoning" && !isCompletedStatus$1(item.status) || isActiveOperationalItem(item)));
}
