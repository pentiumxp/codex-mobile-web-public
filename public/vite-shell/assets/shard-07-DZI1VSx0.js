...
var root$3 = rootObject();
root$3.CodexMobileAppEntry = appEntryApi;
var currentScript = root$3.document && root$3.document.currentScript;
...
var viteShellPreviewPage = Boolean(root$3.document && typeof root$3.document.getElementById === "function" && root$3.document.getElementById("codex-vite-shell-preview"));
if (!root$3.__CODEX_MOBILE_VITE_APP_PREVIEW_PAGE__ && !viteShellPreviewPage || loadedByClassicAppPreview) startCodexMobileApp();
//#endregion
//#region frontend/native/thread-list-runtime.mjs
...
function createThreadListRuntime(deps = {}) {
	...
	const api = deps.api;
	const document = deps.document || root$2.document || {};
	const window = deps.window || root$2.window || root$2;
	const localStorage = deps.localStorage || root$2.localStorage || {
		getItem: () => null,
		setItem: () => {},
		removeItem: () => {}
	};
	const setTimeout = typeof deps.setTimeout === "function" ? deps.setTimeout : root$2.setTimeout.bind(root$2);
	const clearTimeout = typeof deps.clearTimeout === "function" ? deps.clearTimeout : root$2.clearTimeout.bind(root$2);
  ...
  <section class="workspace-stats-section">
	...
	function updateThreadActivityTimestamp(thread, eventAtMs = 0) {
		...
		thread.updatedAt = Math.floor(nextMs / 1e3);
		return true;
	}
...
var api = Object.freeze({ createThreadListRuntime });
root$2.CodexThreadListRuntime = api;
//#endregion
//#region frontend/native/side-chat-runtime.mjs
var root$1 = typeof window !== "undefined" ? window : globalThis;
function noop$1() {}
function noopFalse$1() {
	return false;
}
function noopString$1() {
	return "";
}
function defaultRequestAnimationFrame$1(callback) {
	return typeof root$1.setTimeout === "function" ? root$1.setTimeout(callback, 16) : 0;
}
function createSideChatRuntime(deps = {}) {
	const state = deps.state || {};
	const $ = typeof deps.$ === "function" ? deps.$ : () => null;
	const document = deps.document || root$1.document || {};
	const window = deps.window || root$1.window || root$1;
	const requestAnimationFrame = typeof deps.requestAnimationFrame === "function" ? deps.requestAnimationFrame : typeof root$1.requestAnimationFrame === "function" ? root$1.requestAnimationFrame.bind(root$1) : defaultRequestAnimationFrame$1;
	const setTimeout = typeof deps.setTimeout === "function" ? deps.setTimeout : typeof root$1.setTimeout === "function" ? root$1.setTimeout.bind(root$1) : () => 0;
	const clearTimeout = typeof deps.clearTimeout === "function" ? deps.clearTimeout : typeof root$1.clearTimeout === "function" ? root$1.clearTimeout.bind(root$1) : noop$1;
	...
	const { api, collabAgentNameText = noopString$1, collabAgentTaskText = noopString$1, collabAgentThreadText = noopString$1, conversationDomTurnIds = () => [], conversationPatchShellSignature = noopString$1, conversationRenderSignature = noopString$1, createSubmissionId = () => "sidechat-" + Date.now(), currentLiveTurn = () => null, diagnosticThreadHash = noopString$1, escapeHtml = (value) => String(value || ""), escapeSelectorAttr = (value) => String(value || ""), formatTime = noopString$1, homeAiDiagnosticReportingApi = { boundedToken: (value) => String(value || "") }, isInteractiveGestureTarget = noopFalse$1, latestTurn = () => null, loadThread = async () => null, loadThreads = async () => null, markActivity = noop$1, normalizeClientErrorMessage = (value) => String(value || ""), primaryTouch = () => null, renderCurrentThread = noop$1, requestAppConfirmation = async () => false, scheduleCurrentThreadRefresh = noop$1, scheduleLivePollIfNeeded = noop$1, showError = noop$1, statusText = noopString$1, truncateMiddle = (value) => String(value || ""), visibleConversationShape = () => ({}) } = deps;
...
var sideChatRuntimeApi = Object.freeze({ createSideChatRuntime });
root$1.CodexSideChatRuntime = sideChatRuntimeApi;
//#endregion
//#region frontend/native/media-preview-runtime.mjs
...
function createMediaPreviewRuntime(deps = {}) {
	...
	function handleFilePreviewHtmlViewClick(button) {
		...
		return setFilePreviewHtmlView(button && typeof button.closest === "function" ? button.closest("[data-file-preview-html]") : null, view, button);
	}
	function setFilePreviewHtmlFullscreen(root, fullscreen) {
		...
		if (documentElement && documentElement.classList) documentElement.classList.toggle("file-preview-html-fullscreen-open", fullscreen);
		...
		if (fullscreen) setFilePreviewHtmlView(root, "render", typeof root.querySelector === "function" ? root.querySelector("[data-file-preview-html-view=\"render\"]") : null);
		return true;
	}
	...
	function closeFilePreviewHtmlFullscreen() {
		const root = document && typeof document.querySelector === "function" ? document.querySelector("[data-file-preview-html].is-fullscreen") : null;
...
var mediaPreviewRuntimeApi = Object.freeze({ createMediaPreviewRuntime });
root.CodexMediaPreviewRuntime = mediaPreviewRuntimeApi;
//#endregion
...
var moduleApis = {
	...
	"side-chat-runtime": sideChatRuntimeApi,
	"media-preview-runtime": mediaPreviewRuntimeApi
};
