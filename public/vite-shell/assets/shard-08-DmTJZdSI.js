...
var api$5 = Object.freeze({ createComposerRuntime });
root$2.CodexComposerRuntime = api$5;
//#endregion
...
async function answerServerRequest(requestId, payload, options = {}) {
	...
	try {
		const result = await api$4(`/api/approvals/${encodeURIComponent(key)}`, {
			method: "POST",
...
function answerApproval(requestId, decision, options = {}) {
	...
	return api$4(`/api/approvals/${encodeURIComponent(key)}`, {
		method: "POST",
...
async function mutateThreadTaskCard(cardId, action, body = {}, options = {}) {
	...
	try {
		const result = await api$4(`/api/thread-task-cards/${encodeURIComponent(id)}/${encodeURIComponent(action)}`, {
			method: "POST",
...
async function createThreadTaskCardDraft(draftKey, options = {}) {
	...
	try {
		...
		const result = await api$4("/api/thread-task-cards", {
			method: "POST",
			body: JSON.stringify({
...
async function api$3(path, options = {}) {
...
function returnReceiptTurnIdsForConversation(thread) {
	if (typeof threadTaskCardReturnReceiptTurnIds !== "function") return [];
	return threadTaskCardReturnReceiptTurnIds(thread).map(String).filter(Boolean);
}
function visibleConversationShape(thread) {
	const turns = visibleRenderableTurnsForConversation(thread);
	const returnReceiptTurnIds = typeof returnReceiptTurnIdsForConversation === "function" ? returnReceiptTurnIdsForConversation(thread) : [];
...
function recordEmptyVisibleDetailHealthy(source, thread = state.currentThread, details = {}) {
	...
	return recordHomeAiDiagnosticSuccess$1(threadDiagnosticEventsApi.emptyVisibleDetailMismatchDiagnosticSuccess({
		...
		routeKind: details.routeKind || "single-thread",
		sourceKind: source,
		threadHash: details.threadHash || diagnosticThreadHash$1(threadId),
		readMode: thread.mobileReadMode || "",
		renderMode: details.renderMode || ""
	}));
}
...
function visibleRenderableTurnIds(thread) {
	const returnReceiptTurnIds = typeof returnReceiptTurnIdsForConversation === "function" ? returnReceiptTurnIdsForConversation(thread) : [];
	return visibleRenderableTurnsForConversation(thread).map((turn) => String(turn.id)).concat(returnReceiptTurnIds);
}
...
async function syncExistingPushSubscription() {
	...
	if (subscription) await api$3("/api/push/subscribe", {
		method: "POST",
...
async function enablePushNotifications() {
	...
	if (!subscription) {
		const key = await api$3("/api/push/vapid-public-key");
		subscription = await registration.pushManager.subscribe({
			userVisibleOnly: true,
			applicationServerKey: base64UrlToUint8Array(key.publicKey)
		});
	}
	await api$3("/api/push/subscribe", {
		method: "POST",
		body: JSON.stringify({ subscription: pushSubscriptionToJson(subscription) })
	});
...
async function sendTestPushNotification() {
	const result = await api$3("/api/push/test", {
		method: "POST",
		body: "{}"
	});
	$("connectionState").classList.remove("error");
...
var moduleApis = {
	"composer-runtime": api$5,
	"composer-bridge-runtime": api$4,
	"api-client-runtime": apiClientRuntimeApi,
	"thread-list-load-policy": thread_list_load_policy_default,
	"thread-list-stable-order": api$2,
	"thread-status-hints": api$1,
	"thread-detail-patch-plan": api
};
