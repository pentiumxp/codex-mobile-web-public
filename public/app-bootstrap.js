...
var buildRefreshPolicy = window.CodexBuildRefreshPolicy || {
...
if (!appUpdateRuntimeApi || typeof appUpdateRuntimeApi.createAppUpdateRuntime !== "function") {
  throw new Error("CodexAppUpdateRuntime script failed to load");
}
var sideChatRuntimeApi = window.CodexSideChatRuntime;
if (!sideChatRuntimeApi || typeof sideChatRuntimeApi.createSideChatRuntime !== "function") {
  throw new Error("CodexSideChatRuntime script failed to load");
}
var mediaPreviewRuntimeApi = window.CodexMediaPreviewRuntime;
if (!mediaPreviewRuntimeApi || typeof mediaPreviewRuntimeApi.createMediaPreviewRuntime !== "function") {
  throw new Error("CodexMediaPreviewRuntime script failed to load");
}
...
if (!threadDetailRuntimeApi || typeof threadDetailRuntimeApi.createThreadDetailRuntime !== "function") {
  throw new Error("CodexThreadDetailRuntime script failed to load");
}
...
if (!composerRuntimeApi) {
  throw new Error("CodexComposerRuntime script failed to load");
}
...
var threadListSummaryFromDetailThread = (...args) => (
  requireThreadDetailRuntime().threadListSummaryFromDetailThread(...args)
);

var planThreadOpenCacheReuse = (...args) => (
  requireThreadDetailRuntime().planThreadOpenCacheReuse(...args)
);

var threadHasReusableLoadedDetailState = (...args) => (
  requireThreadDetailRuntime().threadHasReusableLoadedDetailState(...args)
);
...
var MAX_COMMAND_OUTPUT_CHARS = 16000;
var MAX_LIVE_TEXT_CHARS = 60000;
...
var PAGE_REFRESH_CHECK_INTERVAL_MS = 60000;
var PAGE_REFRESH_MIN_CHECK_INTERVAL_MS = 12000;
var HEAVY_VISUAL_RECOVERY_MIN_INTERVAL_MS = 4000;
var PUBLIC_CONFIG_TIMEOUT_MS = 8000;
var PUBLIC_CONFIG_RETRY_DELAYS_MS = [0, 300, 1200];
var THREAD_LOAD_STALL_MS = 12000;
var THREAD_LIST_SLOW_PATH_MS = 1500;
var THREAD_LIST_RUNTIME_STALL_MIN_MS = 1000;
var THREAD_LIST_RUNTIME_STALL_H2_MS = 3000;
var THREAD_LIST_RUNTIME_STALL_REPORT_INTERVAL_MS = 15000;
var PERF_EVENT_THROTTLE_MS = 2000;
var PERF_RENDER_REPORT_MIN_MS = 16;
var PERF_SLOW_RENDER_REPORT_MS = 50;
var PRIMARY_SHELL_CONFLICT_EVIDENCE_MS = 30000;
var EMPTY_DETAIL_HISTORY_RECOVERY_COOLDOWN_MS = 30000;
var RUNNING_THREAD_HINT_STALE_MS = 20 * 60 * 1000;
...
var CONVERSATION_SCROLL_INTENT_MS = 4000;
var AUTOMATIC_CONVERSATION_REFRESH_SOURCES = new Set([
  "scheduled",
  "deferred-refresh",
...
var MERMAID_MIN_SCALE = 0.65;
var MERMAID_MAX_SCALE = 3.2;
var MERMAID_ZOOM_STEP = 0.2;
var SIDE_CHAT_DRAFT_SAVE_DEBOUNCE_MS = 450;
var SIDE_CHAT_DRAFT_MAX_CHARS = 8000;
var COMPOSER_INTENT_BODY_MAX_CHARS = 12000;
...
var THREAD_TASK_CARD_DRAFT_CREATE_STALE_MS = 45000;
var THREAD_TASK_CARD_DRAFT_CREATE_MAX_ATTEMPTS = 3;
var THREAD_TASK_CARD_BODY_MAX_CHARS = 8000;
...
var THREAD_TILE_USER_MAX_PANES = Math.max(1, Math.floor(Number(
...
var THEME_VALUES = new Set(["system", "dark", "light"]);
...
var SIDEBAR_EDGE_OPEN_RATIO = 0.22;
var SUBAGENT_EDGE_SWIPE_PX = 56;
var SUBAGENT_EDGE_SWIPE_MAX_PX = 88;
var SUBAGENT_EDGE_SWIPE_RATIO = 0.08;
...
var IMAGE_PREVIEW_MIN_SCALE = 0.5;
var IMAGE_PREVIEW_MAX_SCALE = 4;
var OPERATIONAL_ITEM_TYPES = new Set(["commandExecution", "collabAgentToolCall", "fileChange", "dynamicToolCall", "mcpToolCall"]);
var HIDDEN_SERVER_REQUEST_METHODS = new Set(["item/tool/call"]);
var USER_INPUT_REQUEST_METHODS = new Set(["item/tool/requestUserInput", "mcpServer/elicitation/request"]);
...
function installClassicGlobalBindings(root, names) {
  ...
  for (const name of uniqueNames) {
    try {
      ...
      // Some host globals may be non-rebindable; object-property export remains the fallback.
    }
  }
}
...
function classicGlobalBindingNamesFromManifest() {
  ...
  const entries = manifest && Array.isArray(manifest.classicGlobalExports) ? manifest.classicGlobalExports : [];
  return entries.flatMap((entry) => Array.isArray(entry && entry.globals) ? entry.globals : []);
}

(function exposeCodexAppBootstrap(root) {
  const appBootstrapApi = {
