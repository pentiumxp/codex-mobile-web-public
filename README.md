...
单行；只有超过物理可容纳列数时才换行。PWA shell cache 升级到
`codex-mobile-shell-v426`。
...
### 6. v426/v425/v424/v423/v422/v421/v420/v419/v418/v417/v416/v415/v414/v413/v412/v411/v410/v409/v408 重构进展

本轮包含一个 server-only 架构补充：thread detail read orchestration 已抽到
`adapters/thread-detail-read-orchestration-service.js`。随后新增 v421-v426 前端/服务端状态修正，
PWA shell cache 升级到 `codex-mobile-shell-v426`。
...
- 中文说明：v426（本次修复）修正 v425 之后第 5 个平铺窗口在足够宽屏幕上仍被迫换行的问题。桌面物理列数不再硬卡 4 列，而是按实际宽度和 user pane ceiling 计算；自动模式仍用 recommended max，tablet 横屏仍保持 4 列上限。PWA shell cache 升级到 `codex-mobile-shell-v426`。
