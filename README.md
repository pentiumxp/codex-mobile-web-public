...
| `CODEX_MOBILE_ROLLOUT_ENRICHMENT_CONTEXT_BYTES` | Server-side rollout tail bytes used only for thread-detail enrichment such as item timestamps, tool-output image attribution, and usage summaries when the rollout is too large for a full runtime scan, default `33554432`. This does not increase the client turn payload. |
