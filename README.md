...
- 中文说明：v247 合并 public PR #54，修复动态投影重复显示 Mobile 用户消息。动态线程详情投影现在会把 `mux-user-*` / Mobile pending 用户 echo 与后续 app-server durable `userMessage` 按规范化文本和上传路径匹配；匹配到真实用户消息后会移除被遮蔽的 mux echo，避免刷新或投影窗口里短暂出现两张同内容用户消息卡。不匹配的 pending echo 会继续保留，防止真实未落库的发送内容被误删。该修复只更新服务端投影逻辑和回归测试，不改变 PWA shell cache；更新后需要重启 8787 Node listener 才会加载新的 projection 代码。本次 public 发布只包含公开源码、README 和测试；没有复制 `.agent-context`、runtime state、本地密钥、上传内容或机器特定诊断。
