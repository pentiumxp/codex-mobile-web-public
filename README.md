...
`thread/start` and `turn/start`. This is the model-visible path for ordinary Codex turns: the
...
current visible thread for that workspace. The tool response is serialized for
the Codex app-server tool-call protocol. Source-thread task-card creation only
...
All task-card injected target turns also receive the MCP-prefixed task-card tool
`mcp__codex_mobile.return_to_source` and a script fallback instruction for
