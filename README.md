...
### 6. v414/v413/v412/v411/v410/v409/v408 重构进展（v414 已部署 Mac production，未推 public）
...
- pane 内独立输入、命令框和操作气泡被记录为下一阶段的真正分屏 runtime 目标，不放进
  当前只读 tile 热修。每个 pane 要被视为一个独立的手机单线程窗口：pane-local
  composer、draft、发送、审批、interrupt、operation bubble/dock、active turn、滚动和
  实时刷新都要隔离；不能仅在现有全局 composer 或全局命令框上加 UI。
