...
| `CODEX_MOBILE_THREAD_LIST_FALLBACK_CACHE_TTL_MS` | Milliseconds to reuse the expensive state DB / rollout / session-index fallback merge for thread lists, default `30000`. Silent list refreshes that run while a thread detail request is in flight use `fallback=defer`, and the server also defers ordinary list fallback when an active detail request is being served, so cold rollout scans do not contend with first paint or live detail refreshes. |
