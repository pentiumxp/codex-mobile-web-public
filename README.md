...
层会返回 `task_card_terminal_no_return_required`。`mcp__codex_mobile.return_to_source`
...
- Codex Mobile task-card toolset 以前只提供 `delegate_to_thread`，
...
- 模型可见 MCP-prefixed task-card tool 新增 `mcp__codex_mobile.return_to_source`，不依赖
...
- `mcp__codex_mobile.return_to_source` 和 `scripts/return-thread-task-card.js`
...
- 中文说明：server-only 修正 Windows public 新建工作区后无法新建 Thread 的问题。根因是普通 `thread/start` / `turn/start` 把 `mcp__codex_mobile.*` 当作 app-server `dynamicTools` 下发，而当前 Codex app-server 会把 MCP-prefixed namespace 判为保留 namespace，报 `dynamic tool namespace is reserved ...`。现在普通新 Thread 和 task-card target turn 都不再通过 `dynamicTools` 暴露 `mcp__codex_mobile.*`；跨线程发卡、`return_to_source` 和 `task_card_heartbeat` 继续通过已注册 `[mcp_servers.codex_mobile]` MCP server 或脚本 fallback 使用。本次不改变 PWA shell cache。
...
- 中文说明：server-only 增加 Codex 线程自己的 `[mcp_servers.codex_mobile]` MCP server config。服务端启动、读取 Profile 列表、工作区创建和 Profile 切换时会自动检查所有已知/目标 `CODEX_HOME/config.toml`，没有 `[mcp_servers.codex_mobile]` 或配置指向旧脚本时就注册/修正 `scripts/codex-mobile-mcp-server.js`；模型可见工具名由 Codex 暴露为 `mcp__codex_mobile.*`，其中 `delegate_to_thread` 复用现有任务卡 API 发 source-direct 卡。注册器同时写入工具的 `approval_mode = "approve"`，避免只读列表和受运行时开关约束的发卡工具被 Codex MCP 权限层重复弹窗。配置只保存脚本路径、server URL 和 key-file 路径，不保存 raw key。本次不改变 PWA shell cache。
- 中文说明：server-only 给 `跨工作区委派` 增加 bounded 诊断日志。开启后 `thread/start` / `turn/start` / `thread/resume` 出站 RPC 会记录是否带有 `mcp__codex_mobile.delegate_to_thread`、工具数量、fallback 指令是否存在，以及 sandbox/approval 概要；`item/tool/call` 回调会记录工具名、target 引用数量、是否有 body 和 outcome。日志不记录用户正文、完整 developer instructions、访问 key 或完整任务卡内容，用于区分“没有注入”“MCP-prefixed 工具不可见”和“模型未调用/调用失败”。本次不改变 PWA shell cache。
...
- 中文说明：server-only 给 `跨工作区委派` 增加模型可见脚本 fallback。开启后 `thread/start` / `turn/start` 除了注入 `mcp__codex_mobile.delegate_to_thread`，还会在开发者指令里写明：如果动态工具不可见或不可发现，源线程必须运行 `scripts/create-thread-task-card.js` 创建任务卡；`multi_agent_v1.*` 不是 Codex Mobile 任务卡 API，不能替代跨工作区发卡。本次不改变 PWA shell cache。
- 中文说明：server-only 明确 `跨工作区委派` 的失败后恢复边界。开启后模型如果已经误尝试目标工作区写入/命令/部署并遇到 sandbox、permission denied、operation not permitted、cwd 或 approval-policy 失败，动态工具说明会要求源线程模型结合当前上下文自行决定并调用 `mcp__codex_mobile.delegate_to_thread`；服务端不会从失败日志后台代发任务卡，避免丢失原线程上下文和意图。本次不改变 PWA shell cache。
...
- 中文说明：server-only 收紧 `跨工作区委派` 的模型可见工具说明和审批语义。开关开启后，`mcp__codex_mobile.delegate_to_thread` 的描述明确要求：如果用户请求的实现、文件修改、命令、测试、部署或其他状态变更属于另一个工作区/线程，模型必须先调用该工具创建任务卡，不得在当前线程里直接 `cd`、读写、打补丁、运行命令或部署目标工作区。该动态工具路径固定创建 source-direct 卡，不允许模型把自由委派卡发成 Pending；Pending 仍保留给手动/API/MCP 等显式审批路径。仍保持“由模型判断是否跨工作区”，不恢复本地关键词/路径启发式预检。本次不改变 PWA shell cache。
...
- 中文说明：server-only 给已开启 `跨工作区委派` 的 Codex 线程注入模型可见 MCP-prefixed task-card tool `mcp__codex_mobile.delegate_to_thread`。模型在判断当前请求需要另一个工作区/线程处理时，可以显式调用这个工具，服务端复用 `/api/threads/:sourceThreadId/task-cards` 创建任务卡；开关关闭时完全不注入。本次不改变 PWA shell cache。
...
- A plain target-thread final answer is never treated as a source-thread return
  ...
  `POST /api/thread-task-cards/:id/reply`, `mcp__codex_mobile.return_to_source`, or
  `scripts/return-thread-task-card.js`.
- Return cards created through `mcp__codex_mobile.return_to_source`,
...
When the same runtime switch is enabled, Codex Mobile syncs the standard
`codex_mobile` MCP server and appends guidance for the MCP-prefixed task-card
tool `mcp__codex_mobile.delegate_to_thread` to `thread/start` and `turn/start`.
It does not send `mcp__codex_mobile.*` through app-server `dynamicTools`,
because current Codex app-server reserves MCP-prefixed namespaces. This is the
model-visible path for ordinary Codex turns: the model decides whether a request
needs another workspace/thread, calls the MCP tool with an exact target thread
id/title or exact target cwd, and the server creates the task card through the
same source-thread route. Exact thread ids/titles are thread identity, so
multiple normal threads may share one cwd; archived, deleted, hidden, subagent,
or non-detail-readable targets are still rejected. `targetCwd` /
`targetWorkspace` remain fuzzy workspace targets and choose a current visible
thread for that workspace. Source-thread task-card creation only accepts
deliverable target threads and rejects self-cards explicitly.
The same start/turn runtime guidance also includes a local script fallback for
agents that do not see the MCP-prefixed tool after discovery. If no direct callable
`mcp__codex_mobile.delegate_to_thread` tool surface is visible, the agent should use
...
All task-card injected target turns also receive guidance for the MCP-prefixed
task-card tool `mcp__codex_mobile.return_to_source` and a script fallback
instruction for `scripts/return-thread-task-card.js`. The injected task-card
message includes
...
Codex Mobile also registers a standard Codex MCP server config named
`codex_mobile`; Codex exposes its model-visible tools as `mcp__codex_mobile.*`.
