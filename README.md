...
### 2026-06-01 Public 发布说明（额度来源、Token 统计和冷启动加载 v153）

本次 public 同步 private 已验证的 v147-v153 改动，版本仍为 `0.1.11`，PWA shell cache 升到 `codex-mobile-shell-v153`。更新后需要重启 8787 Node listener；已经打开的浏览器、PWA 或 Hermes 插件 iframe 需要接受刷新提示、硬刷新、关闭重开或由宿主触发插件刷新，才能加载新的静态前端。

- 额度显示改为只采用当前 active profile 的 `/api/public-config` 和 `/api/status` 快照。来自其他 workspace 或其他 profile 的 app-server `account/rateLimits/updated` 事件不会直接覆盖当前界面，避免同一线程里 5 小时/周额度在不同账号或不同模型组之间跳动。
- Workspace Token 统计继续写入本地 SQLite，并在侧栏保留紧凑的 `总/周/今` 汇总；显示单位改为百万。全屏统计页按日期和项目拆分 uncached input、cached input、output 和 reasoning output，线程列表不再显示每个线程的今日 token 徽标，避免列表噪音。
- Workspace 统计会用可见 workspace root 归一化历史 cwd。若旧 SQLite 记录里存在 Windows 路径乱码，服务端会把它合并到正确的可读 workspace，而不是在统计页里显示第二个乱码项目。
- 线程列表默认请求页大小降到 40，减少启动、前台恢复和切换线程时不必要的 thread-list 开销；较大 rollout 的线程详情继续优先走有上限的 `thread/turns/list`，避免先执行昂贵的完整 `thread/read`。
- 冷启动恢复已保存线程时，前端会先显示 `Opening thread...`，并把线程详情读取与 status、workspace、thread-list 刷新并行执行。启动阶段会记录 `startup_stage` 诊断事件，便于从本地日志区分 public-config、status、workspace、列表、detail 和渲染耗时。
- `pageshow`、focus 等恢复事件如果发生在启动过程中，只做视觉恢复并记录 `mobile_resume_skipped_startup`，不会再额外跑完整 network resume。这样可以避免 bootstrap 正在打开线程时重复拉 status/list/detail，降低“先停在壳层、再停在 Loading thread”的双段停顿。
- 本次同步更新 `server.js`、`adapters/token-usage-stats-service.js`、`public/index.html`、`public/app.js`、`public/styles.css`、`public/sw.js`、README、架构/模块/故障排除文档以及相关 UI/route/token 统计测试。发布同步没有复制 `.agent-context`、runtime state、本地密钥、上传内容或机器特定诊断。
