...
### 2026-06-01 Public 发布说明（工作区 Token 统计与刷新提示 v146）

本次 public 同步 private 已验证的工作区 Token 统计和页面刷新提示修正。版本仍为 `0.1.11`，PWA shell cache 升到 `codex-mobile-shell-v146`。部署后需要重启 8787 Node listener；已打开的浏览器/PWA 需要接受刷新提示、硬刷新或关闭重开，才能加载新的统计页面和刷新检测逻辑。

- 新增实时 SQLite Token 台账：完成 turn 后，服务端把 scoped Usage 写入 `%USERPROFILE%\.codex-mobile-web\token-usage-stats.sqlite`，按 `(thread_id, turn_id)` upsert，避免重复 completion 事件双算。
- 统计按 Workspace `cwd` 落库并聚合。侧栏 Workspace 下方显示红色紧凑 `总/周/今` 行，点击 `统计` 打开全屏统计页。
- 全屏统计页包含总览、按天统计和按项目统计，并把 Uncached input、Cached input、Output、Reasoning output 分开显示，因为这些 token 类型的成本和意义不同。
- 完成回执里的 Usage 统计继续按同一 turn 内连续 `total_token_usage` 差值累计，避免长 turn 只显示最后一次模型调用的 token。
- 页面刷新提示只在服务端真正重启到新的 startup-captured shell build 后出现；前端还会在 EventSource 重新打开、状态事件和线程列表刷新成功后主动检查新 build，减少真实升级后不提示刷新的情况。
- 本次同步更新 `adapters/token-usage-stats-service.js`、SQLite helper、Usage summary 服务、`server.js`、`public/app.js`、`public/styles.css`、`public/sw.js`、README/docs 和相关测试。未同步 `.agent-context`、runtime state、本地密钥、上传内容或机器特定诊断。
