...
- 中文说明：长对话如果因为恢复、切换线程或手动滚动停在历史消息中间，页面会在输入框上方显示“回到底部”浮动按钮。按钮只在当前线程已加载、内容可滚动且不在底部时出现；点击后立即回到最新 turn。用户阅读历史内容时，普通刷新仍不会强制自动滚到底部。PWA/手机浏览器如果仍显示旧界面，需要刷新一次或等待新的 service worker 缓存 `codex-mobile-shell-v14` 激活。
...
- On coarse-pointer landscape tablets, the composer now uses a two-row compact layout: runtime indicators and quota on the first row, then attach/input/send on the second row. This prevents control clipping in iPad landscape split widths and improves keyboard overlap handling.
...
交接文件属于本地运行态资料。创建交接目录时，服务端会在 `.agent-context/thread-handoffs/.gitignore` 写入忽略规则，防止这些自动生成的 Markdown 交接文件被误提交。
...
- The composer keeps `Permission` as the only editable runtime selector on mobile.
- `Model` and `Reasoning` are read-only displays sourced from the current thread/default runtime, to avoid mobile-side edits diverging from Desktop thread settings.
- The composer shows 5-hour and weekly quota remaining when app-server sends rate-limit updates. Rate-limit updates are cached by model key, and mobile quota display follows the thread/default model shown in the read-only field.
...
- Mobile Web no longer submits `model` or `effort` fields for normal send, continuation, or new-thread-first-message requests.
