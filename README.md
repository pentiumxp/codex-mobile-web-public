...
1. **大 session 性能闭环**：第一步已经把 thread detail read orchestration
   ...
   大 rollout 的问题。后续继续采集重启/冷开和 warm cache 的分层耗时证据，
   分别测量 thread list fallback、thread detail summary、projection seed/cache、
...
### 6. v419/v418/v417/v416/v415/v414/v413/v412/v411/v410/v409/v408 重构进展（v419 已部署 Mac production，本次同步 public）

本次 public 同步还包含一个 server-only 架构补充：thread detail read
orchestration 已抽到 `adapters/thread-detail-read-orchestration-service.js`。
因为没有修改前端静态 shell，PWA shell cache 仍保持 `codex-mobile-shell-v419`。
