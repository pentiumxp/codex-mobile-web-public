...
### 6. v425/v424/v423/v422/v421/v420/v419/v418/v417/v416/v415/v414/v413/v412/v411/v410/v409/v408 重构进展

本轮包含一个 server-only 架构补充：thread detail read orchestration 已抽到
`adapters/thread-detail-read-orchestration-service.js`。随后新增 v421-v425 前端/服务端状态修正，
PWA shell cache 升级到 `codex-mobile-shell-v425`。
...
- 平铺手动窗口数使用独立 user pane ceiling；视口推荐容量继续控制自动模式，
  但不再阻止用户在宽桌面上继续新增窗口。超过推荐列数时，pane 网格换行显示。
...
- 中文说明：v425（本次修复）把 Home AI embedded notification/task-card route hint 的打开、聚焦、缺失回退和 URL scrub 策略抽到 `public/plugin-embed.js` 并加可执行测试；平铺手动窗口数不再被设备推荐容量卡在 4，用户新增窗口可达到 bounded user pane ceiling，超过推荐列数时换行显示。PWA shell cache 升级到 `codex-mobile-shell-v425`。
