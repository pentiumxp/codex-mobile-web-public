...
- 中文说明：server-only 修正后台 turn 已经启动但线程列表不显示运行状态的问题。Mobile Web 自己发起的 `turn/start` 成功返回后，现在会立即广播轻量 `thread/status/changed active`，同时更新线程详情投影缓存；覆盖普通消息、source-direct/自动任务卡注入、auto-recover、side-chat apply、continuation handoff/bootstrap 和新线程首 turn。这样目标线程不在当前页面、或 app-server 原始 `turn/started` 通知延迟/缺失时，侧边栏/首页仍能及时显示已启动。本次不改变 PWA shell cache。
